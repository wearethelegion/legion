...
import { createLegionClient } from "@wearethelegion/sdk"
...
const { client, server } = createLegion()
...
try {
  ...
  console.log("legion session", session.id)
  if (shareId) {
...
function createLegion() {
  ...
  const proc = spawn(`legion`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createLegionClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/legion|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/legion` or `/oc`")
  }
}
...
async function assertLegionConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to legion server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.wearethelegion.com" : "https://wearethelegion.com"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.wearethelegion.com/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("legion-github-action")
    response = await fetch("https://api.wearethelegion.com/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/legion" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/legion") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to legion...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "legion-agent[bot]"`
  await $`git config --global user.email "legion-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `legion/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/legion-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[legion session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
