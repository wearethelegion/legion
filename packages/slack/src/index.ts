...
import { createLegion, type ToolPart } from "@wearethelegion/sdk"
...
console.log("🚀 Starting legion server...")
const legion = await createLegion({
  port: 0,
})
console.log("✅ Legion server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await legion.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new legion session...")
    const { client, server } = legion
    ...
    console.log("✅ Created legion session:", createResult.data.id)
  ...
  console.log("📝 Sending to legion:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Legion response:", JSON.stringify(result, null, 2))
