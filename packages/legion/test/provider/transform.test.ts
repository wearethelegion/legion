...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const legionModel = {
      ...
      api: {
        id: "legion-test",
        url: "https://api.wearethelegion.com",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, legionModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.legion?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.legion?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.legion?.itemId).toBe("msg_legion")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.legion?.itemId).toBe("msg_legion_part")
