...
const managedConfigDir = process.env.LEGION_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "legion.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "legion.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "legion.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .legion directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const legionDir = path.join(dir, ".legion")
      await fs.mkdir(legionDir, { recursive: true })
      const agentDir = path.join(legionDir, "agent")
...
test("loads agents from .legion/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const legionDir = path.join(dir, ".legion")
      await fs.mkdir(legionDir, { recursive: true })

      const agentsDir = path.join(legionDir, "agents")
...
test("loads commands from .legion/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const legionDir = path.join(dir, ".legion")
      await fs.mkdir(legionDir, { recursive: true })

      const commandDir = path.join(legionDir, "command")
...
test("loads commands from .legion/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const legionDir = path.join(dir, ".legion")
      await fs.mkdir(legionDir, { recursive: true })

      const commandsDir = path.join(legionDir, "commands")
...
test("does not try to install dependencies in read-only LEGION_CONFIG_DIR", async () => {
  ...
  const prev = process.env.LEGION_CONFIG_DIR
  process.env.LEGION_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.LEGION_CONFIG_DIR
    else process.env.LEGION_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable LEGION_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "legion.json"),
        JSON.stringify({ $schema: "https://wearethelegion.com/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "legion.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .legion config
      const projectDir = path.join(dir, "project")
      const legionDir = path.join(projectDir, ".legion")
      await fs.mkdir(legionDir, { recursive: true })
      ...
      // Local .legion config with different plugins
      await Bun.write(
        path.join(legionDir, "legion.json"),
        JSON.stringify({
          $schema: "https://wearethelegion.com/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .legion config with some overlapping plugins
      await Bun.write(
        path.join(legionDir, "legion.json"),
        JSON.stringify({
          $schema: "https://wearethelegion.com/config.json",
...
// Note: preload.ts sets LEGION_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .legion config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .legion directory config enables it
      const legionDir = path.join(dir, ".legion")
      await fs.mkdir(legionDir, { recursive: true })
      await Bun.write(
        path.join(legionDir, "legion.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/legion")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/legion")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-legion@2.4.3")).toBe("oh-my-legion")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@legion/plugin@2.0.0")).toBe("@legion/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-legion@2.4.3", "file:///project/.legion/plugin/oh-my-legion.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.legion/plugin/oh-my-legion.js")
  })
  ...
  test("local plugin directory overrides global legion.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const legionDir = path.join(projectDir, ".legion")
        const pluginDir = path.join(legionDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("LEGION_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["LEGION_DISABLE_PROJECT_CONFIG"]
    process.env["LEGION_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["LEGION_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["LEGION_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .legion/ directories when flag is set", async () => {
    const originalEnv = process.env["LEGION_DISABLE_PROJECT_CONFIG"]
    process.env["LEGION_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .legion directory with a command
          const legionDir = path.join(dir, ".legion", "command")
          await fs.mkdir(legionDir, { recursive: true })
          await Bun.write(path.join(legionDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["LEGION_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LEGION_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["LEGION_CONFIG_DIR"]
      process.env["LEGION_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["LEGION_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["LEGION_CONFIG_DIR"]
      } else {
        process.env["LEGION_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("LEGION_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["LEGION_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["LEGION_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["LEGION_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["LEGION_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from LEGION_CONFIG_DIR, not project
