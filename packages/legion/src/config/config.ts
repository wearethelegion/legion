...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/legion"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "legion")
      default:
        return "/etc/legion"
    }
  }

  const managedConfigDir = process.env.LEGION_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  ...
  export const state = Instance.state(async () => {
    ...
    // 1) Remote .well-known/legion (org defaults)
    // 2) Global config (~/.legion/legion.json{,c} or ~/.legion/config.json)
    // 3) Custom config (LEGION_CONFIG)
    // 4) Project config (legion.json{,c})
    // 5) .legion directories (.legion/agents/, .legion/commands/, .legion/plugins/, .legion/legion.json{,c})
    // 6) Inline config (LEGION_CONFIG_CONTENT)
    // Managed config directory is enterprise-only and always overrides everything above.
    let result: Info = {}
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        process.env[value.key] = value.token
        log.debug("fetching remote config", { url: `${key}/.well-known/legion` })
        const response = await fetch(`${key}/.well-known/legion`)
        if (!response.ok) {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://wearethelegion.com/config.json"
        result = merge(result, await load(JSON.stringify(remoteConfig), `${key}/.well-known/legion`))
    ...
    if (Flag.LEGION_CONFIG) {
      result = merge(result, await loadFile(Flag.LEGION_CONFIG))
      log.debug("loaded custom config", { path: Flag.LEGION_CONFIG })
    }

    // Project config overrides global and remote config.
    if (!Flag.LEGION_DISABLE_PROJECT_CONFIG) {
    ...
    const directories = [
      ...
      ...(!Flag.LEGION_DISABLE_PROJECT_CONFIG
    ...
    if (Flag.LEGION_CONFIG_DIR) {
      directories.push(Flag.LEGION_CONFIG_DIR)
    ...
    for (const dir of unique(directories)) {
      if (dir.endsWith(".legion") || dir.endsWith(".opencode") || dir === Flag.LEGION_CONFIG_DIR) {
        for (const file of ["legion.jsonc", "legion.json", "opencode.jsonc", "opencode.json"]) {
    ...
    if (Flag.LEGION_CONFIG_CONTENT) {
      result = merge(result, JSON.parse(Flag.LEGION_CONFIG_CONTENT))
      log.debug("loaded custom config from LEGION_CONFIG_CONTENT")
    }
    ...
    // The parent serialises its resolved mcp section as LEGION_MCP_CONFIG_OVERRIDE
    ...
    // if (process.env.LEGION_MCP_CONFIG_OVERRIDE) {
    //   try {
    //     const parentMcp = JSON.parse(process.env.LEGION_MCP_CONFIG_OVERRIDE) as Record<string, unknown>
    //     result.mcp = { ...parentMcp, ...result.mcp } as Info["mcp"]
    //     log.info("merged parent MCP config from LEGION_MCP_CONFIG_OVERRIDE", {
    //       servers: Object.keys(parentMcp).join(", "),
    //     })
    //   } catch (err) {
    //     log.warn("failed to parse LEGION_MCP_CONFIG_OVERRIDE — ignoring", {
    ...
    if (Flag.LEGION_PERMISSION) {
      result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.LEGION_PERMISSION))
    }
    ...
    if (Flag.LEGION_DISABLE_AUTOCOMPACT) {
      result.compaction = { ...result.compaction, auto: false }
    }
    if (Flag.LEGION_DISABLE_PRUNE) {
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@wearethelegion/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@wearethelegion/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@wearethelegion/plugin",
   ...
   * getPluginName("oh-my-legion@2.4.3") // "oh-my-legion"
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-legion", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-legion@2.4.3", "file:///path/to/plugin.js"
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: legion.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for legion serve and web commands"),
      command: z
        .record(z.string(), Command)
        .optional()
        .describe("Command configuration, see https://wearethelegion.com/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://wearethelegion.com/docs/agents"),
      provider: z
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://wearethelegion.com/config.json"
  ...
  async function load(text: string, configFilepath: string) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema) {
        parsed.data.$schema = "https://wearethelegion.com/config.json"
