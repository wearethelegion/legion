...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.LEGION_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no LEGION_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.LEGION_CONFIG_DIR, Flag.LEGION_CONFIG_DIR).catch(() => [])
}
