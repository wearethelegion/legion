...
export namespace ACP {
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const legionProvider = providers.find((p) => p.id === "legion")
    if (legionProvider) {
      if (legionProvider.models["big-pickle"]) {
        return { providerID: "legion", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(legionProvider.models))
      if (best) {
        return {
