...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for legion.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal legion sessions
...
legion acp --cwd /path/to/project
```
...
```bash
LEGION_ENABLE_QUESTION_TOOL=1 legion acp
```
...
- **Terminal Integration**: Full terminal support via legion's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | legion acp
```
...
ACP sessions map cleanly to legion's internal session model:
