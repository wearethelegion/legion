...
import { fn } from "@wearethelegion/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Bun.file(path.join(dotgit, "legion"))
          .text()
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.LEGION_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Bun.file(path.join(dotgit, "legion"))
              .write(id)
              .catch(() => undefined)
          }
        }
    ...
    if (Flag.LEGION_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
