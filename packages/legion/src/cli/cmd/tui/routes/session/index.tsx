...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@wearethelegion/sdk/v2"
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  █     ${UI.Style.TEXT_DIM}legion -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        `  ▀▀▀▀  `,
        `${logo[0] ?? ""}`,
        `${logo[1] ?? ""}`,
        `${logo[2] ?? ""}`,
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}legion -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.LEGION_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
