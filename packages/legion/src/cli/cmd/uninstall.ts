...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall legion and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g legion",
      pnpm: "pnpm uninstall -g legion",
      bun: "bun remove -g legion",
      yarn: "yarn global remove legion",
      brew: "brew uninstall legion",
      choco: "choco uninstall legion",
      scoop: "scoop uninstall legion",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall legion -y -r`.quiet().nothrow()
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".legion") || binDir.includes(".opencode")) {
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# legion" || trimmed === "# legion") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".legion/bin") || trimmed.includes(".opencode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".legion/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".legion") || trimmed.includes(".opencode")))
    ) {
      continue
    }
