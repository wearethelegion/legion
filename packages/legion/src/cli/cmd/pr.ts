...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run legion",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for legion session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found legion session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`legion import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting legion...")
        UI.println()

        // Launch legion TUI with session ID if available
        const { spawn } = await import("child_process")
        const legionArgs = sessionId ? ["-s", sessionId] : []
        const legionProcess = spawn("legion", legionArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          legionProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`legion exited with code ${code}`))
          })
          legionProcess.on("error", reject)
        })
      },
    })
  },
