...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run legion with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      const sdk = createLegionClient({ baseUrl: args.attach, directory })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createLegionClient({ baseUrl: "http://legion.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
