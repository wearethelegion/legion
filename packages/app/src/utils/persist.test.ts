...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("legion.throw")) throw new Error("storage get failed")
  ...
  setItem(key: string, value: string) {
    ...
    if (key.startsWith("legion.quota")) throw new DOMException("quota", "QuotaExceededError")
    if (key.startsWith("legion.throw")) throw new Error("storage set failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("legion.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("legion.quota.scope")
    storageApi.setItem("value", '{"value":1}')

    expect(storage.getItem("legion.quota.scope:value")).toBeNull()
    expect(storageApi.getItem("value")).toBeNull()
  })

  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("legion.throw.scope")
    ...
    const healthy = persistTesting.localStorageWithPrefix("legion.safe.scope")
    healthy.setItem("value", '{"value":3}')
    expect(storage.getItem("legion.safe.scope:value")).toBe('{"value":3}')
  })

  test("failing fallback scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("legion.throw.scope2")
