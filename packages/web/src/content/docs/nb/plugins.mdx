...
description: Skriv dine egne programtillegg for å utvide legion.
---

Plugins lar deg utvide Legion ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Legion.
...
- `.legion/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/legion/node_modules/`.
...
1. Global konfigurasjon (`~/.config/legion/legion.json`)
2. Prosjektkonfigurasjon (`legion.json`)
3. Global plugin-katalog (`~/.config/legion/plugins/`)
4. Prosjektpluginkatalog (`.legion/plugins/`)
...
Legion kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Legion SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Legion.
...
:::note
Hvis du bruker Legion-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Legion fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Legion:
...
`tool`-hjelperen lager et tilpasset verktøy som Legion kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Legion sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://legion.dev/docs/sdk) for detaljer.
