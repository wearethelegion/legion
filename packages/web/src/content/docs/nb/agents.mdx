...
Det er to typer agenter i Legion; primære agenter og underagenter.
...
Legion kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Legion kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Legion kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `legion.json`:
...
- Globalt: `~/.config/legion/agents/`
- Per prosjekt: `.legion/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Legion modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Legion-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Legion-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Legion Zen](/docs/zen), vil du bruke `legion/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `legion models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/wearethelegion/legion).
:::
