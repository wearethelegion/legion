---
title: LSP-servere
description: legion integreres med LSP-serverne dine.
---

Legion integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

Legion kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `LEGION_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når legion åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i Legion-konfigurasjonen.
