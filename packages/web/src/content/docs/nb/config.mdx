---
title: Konfigurasjon
description: Ved å bruke legion JSON-konfigurasjonen.
---

Du kan konfigurere Legion ved å bruke en JSON konfigurasjonsfil.

---

## Format

Legion støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "legion"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/legion`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/legion/legion.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`LEGION_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`legion.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.legion` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`LEGION_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.legion` og `~/.config/legion` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/legion`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Legion-konfigurasjon i `~/.config/legion/legion.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `legion.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Legion starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `LEGION_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `LEGION_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.legion` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.legion`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`legion.dev/config.json`**](https://legion.dev/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `legion serve` og `legion web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Legion-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `legion.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Legion-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Legion å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan konfigurere temaet du vil bruke i Legion-konfigurasjonen gjennom alternativet `theme`.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/legion/agents/` eller `.legion/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Legion falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`legion run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/legion/commands/` eller `.legion/commands/`. [Les mer her](/docs/commands).
...
Legion vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Legion alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Legion med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.legion/plugins/` eller `~/.config/legion/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Legion til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
