...
Du kan legge til eksterne verktøy i Legion ved å bruke _Model Context Protocol_ eller MCP. Legion støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Legion Config](https://legion.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/legion`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Legion håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Legion:
...
Hvis serveren krever autentisering, vil Legion be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `legion mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Legion lagre tokens sikkert i `~/.local/share/legion/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Legion, sammen med innebygde verktøy. Så du kan administrere dem gjennom Legion-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Legion til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Legion-samtale oppført som en separat økt.
