---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Legion-konfigurasjonen.
...
i `~/.local/share/legion/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Legion
konfig.
...
Legion Zen er en liste over modeller levert av Legion-teamet som har vært
testet og verifisert for å fungere godt med Legion. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Legion Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg legion og gå til [legion.dev/auth](https://legion.dev/auth).
...
Det fungerer som alle andre leverandører i Legion og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med Legion:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Legion:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `legion.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i legion støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Legion skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Legion-konfigurasjonen.
...
for å lagre token i Legion auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Legion bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Legion
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`legion.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `legion.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Legion:
...
Slik bruker du Google Vertex AI med Legion:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Legion.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Legion, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) plugin for automatisk å logge hver Legion-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Legion-samtale oppført som en separat økt.
...
Du kan konfigurere Legion for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Legion for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Legion for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Legion. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/legion) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Legion:
...
3. Kopier API-nøkkelen for bruk i Legion.
...
6. **Viktig**: Før du bruker skymodeller i Legion, må du hente modellinformasjonen lokalt:
...
Legion Zen er en liste over testede og verifiserte modeller levert av Legion-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Legion Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Legion Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Legion API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Legion-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Legion-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Legion:
...
Du kan også tilpasse modeller gjennom Legion-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Legion. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `legion.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Legion forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `legion auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Legion-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Legion-konfigurasjonen.
