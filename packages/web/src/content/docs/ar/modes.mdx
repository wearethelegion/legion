...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات legion. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في legion تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات legion.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.legion/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `legion.json` لديك:
...
- عالمي: `~/.config/legion/modes/`
- للمشروع: `.legion/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم legion القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات legion العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.legion/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/legion/modes/` لأوضاع عالمية:
