---
title: الإعدادات
description: استخدام ملف إعدادات Legion بصيغة JSON.
---

يمكنك ضبط Legion باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم Legion تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "legion"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/legion`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/legion/legion.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`LEGION_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`legion.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.legion`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`LEGION_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.legion` و`~/.config/legion` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/legion`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات Legion العامة في `~/.config/legion/legion.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `legion.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.

:::tip
ضع إعدادات المشروع الخاصة في جذر مشروعك.
:::

عند تشغيل Legion، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `LEGION_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `LEGION_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.legion`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.legion`، لذلك **يمكنه تجاوز** إعداداتها.

---

## المخطط

يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`legion.dev/config.json`**](https://legion.dev/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `legion serve` و`legion web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم Legion.
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `legion.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات Legion عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول Legion استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات Legion عبر الخيار `theme`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/legion/agents/` أو `.legion/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود Legion إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`legion run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/legion/commands/` أو `.legion/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
سيقوم Legion بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
افتراضيًا يسمح legion **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
[الإضافات](/docs/plugins) توسّع Legion بأدوات وخُطافات وتكاملات مخصصة.

ضع ملفات الإضافات في `.legion/plugins/` أو `~/.config/legion/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
...
هذا مفيد عندما تريد تقييد Legion لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
