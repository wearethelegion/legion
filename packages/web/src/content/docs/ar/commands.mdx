...
أنشئ الملف `.legion/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Legion أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Legion:
...
- على مستوى النظام: `~/.config/legion/commands/`
- لكل مشروع: `.legion/commands/`
...
يتضمن legion عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
