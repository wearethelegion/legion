...
مع Legion يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Legion سمتنا `legion`.
...
- يريدون أن يطابق Legion مظهر terminal لديهم
...
يدعم Legion نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/legion/themes/*.json` أو `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.legion/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.legion/themes/*.json`
