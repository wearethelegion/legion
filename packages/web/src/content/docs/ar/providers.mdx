---
title: المزوّدون
description: استخدام أي موفّر LLM في Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Legion ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Legion.
...
في `~/.local/share/legion/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Legion.
...
Legion Zen هي قائمة نماذج يوفّرها فريق Legion وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Legion. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Legion Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر legion، ثم انتقل إلى [legion.dev/auth](https://legion.dev/auth).
...
يعمل مثل أي مزوّد آخر في Legion واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع Legion:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل legion:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `legion.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Legion غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل legion بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات legion.
...
تخزين الرمز في مساحة تخزين مصادقة legion.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Legion نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Legion
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`legion.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### التهيئة

خصّص عبر `legion.json`:
...
لاستخدام اشتراك GitHub Copilot مع legion:
...
لاستخدام Google Vertex AI مع Legion:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر legion، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) لتسجيل كل محادثة Legion تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Legion مدرجة كجلسة مستقلة.
...
يمكنك ضبط legion لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط legion لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط legion لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Legion. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/legion) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Legion:
...
3. انسخ مفتاح API لاستخدامه في Legion.
...
6. **مهم**: قبل استخدام نماذج السحابة في Legion، يجب سحب معلومات النموذج محليا:
...
Legion Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Legion. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Legion Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Legion Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Legion.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات legion.
...
5. يمكنك أيضا تخصيصها عبر إعدادات legion. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Legion:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات legion. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع legion. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `legion.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Legion بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `legion auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات legion وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات legion.
