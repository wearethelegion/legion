...
يمكنك إضافة أدوات خارجية إلى Legion باستخدام _Model Context Protocol_، أو MCP. يدعم Legion خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Legion](https://legion.dev/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/legion` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Legion تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Legion بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Legion المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `legion mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Legion الرموز بشكل آمن في `~/.local/share/legion/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Legion إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Legion مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Legion بحساب Sentry الخاص بك.
