---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Legion.
---

تتيح لك الإضافات توسيع Legion عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Legion.
...
- `.legion/plugins/` - إضافات على مستوى المشروع
- `~/.config/legion/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/legion/node_modules/`.
...
1. إعدادات عامة (`~/.config/legion/legion.json`)
2. إعدادات المشروع (`legion.json`)
3. دليل الإضافات العام (`~/.config/legion/plugins/`)
4. دليل إضافات المشروع (`.legion/plugins/`)
...
يشغّل Legion الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ legion للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع legion.
...
:::note
إذا كنت تستخدم تطبيق Legion لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع legion من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى legion:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ legion استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ legion إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://legion.dev/docs/sdk) للتفاصيل.
