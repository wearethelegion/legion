...
:::caution
Modes are now configured through the `agent` option in the legion config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in legion allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the legion config.
...
legion comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.legion/plans/*.md` to detail the plan itself
...
Configure modes in your `legion.json` config file:
...
both the global legion config and the project specific config.
...
Create mode files in `.legion/modes/` for project-specific modes or `~/.config/legion/modes/` for global modes:
