...
There are two types of agents in Legion; primary agents and subagents.
...
Legion comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Legion comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Legion comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `legion.json` config file:
...
If no temperature is specified, Legion uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Legion config and the project specific config.
...
The model ID in your Legion config uses the format `provider/model-id`. For example, if you're using [Legion Zen](/docs/zen), you would use `legion/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `legion models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/wearethelegion/legion).
:::
