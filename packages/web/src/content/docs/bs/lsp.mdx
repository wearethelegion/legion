---
title: LSP serveri
description: Legion se integriše sa vašim LSP serverima.
---

Legion se integriše sa vašim Language Server Protocol (LSP) serverima kako bi pomogao LLM-u u interakciji s kodnom bazom. Koristi dijagnostiku za slanje povratnih informacija modelu.

## Ugrađeni

Legion dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `LEGION_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Možete prilagoditi LSP servere kroz `lsp` odjeljak u vašoj legion konfiguraciji.
