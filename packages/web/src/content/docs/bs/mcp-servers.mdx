...
Možete dodati vanjske alate u Legion koristeći _Model Context Protocol_, ili MCP. Legion podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Legion Config](https://legion.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/legion`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Legion automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Legion će:
...
Ako server zahtijeva autentifikaciju, Legion će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `legion mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Legion sigurno čuva tokene u `~/.local/share/legion/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Legion, zajedno s ugrađenim alatima. Možete njima upravljati kroz Legion konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Legion sa vašim Sentry nalogom.
