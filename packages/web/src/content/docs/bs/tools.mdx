...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Legion dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Legion.
...
:::note
Ovaj alat je dostupan samo kada je `LEGION_EXPERIMENTAL_LSP_TOOL=true` (ili `LEGION_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Legion provajdera ili kada je varijabla `LEGION_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Legion:
