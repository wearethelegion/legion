...
U Legion mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Legion koristi nasu `legion` temu.
...
Legion dolazi sa vise ugradenih tema.
...
- Zele da Legion odgovara izgledu njihovog terminala
...
Legion podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/legion/themes/*.json` ili `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.legion/themes/*.json`
4. **Trenutni radni direktorij** - `./.legion/themes/*.json`
