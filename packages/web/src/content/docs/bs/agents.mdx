...
Postoje dvije vrste agenata u Legion: primarni agenti i podagenti.
...
Legion dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Legion dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Legion dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `legion.json`:
...
- Globalno: `~/.config/legion/agents/`
- Po projektu: `.legion/agents/`
...
Ako temperatura nije navedena, Legion koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Legion konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Legion konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Legion Zen](/docs/zen), koristili biste `legion/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `legion models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/wearethelegion/legion).
:::
