...
ove ili konfigurirajte svoje putem legion config.
...
legion dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.legion/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `legion.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/legion/modes/`
- Projekat: `.legion/modes/`
...
Ako temperatura nije navedena, legion koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.legion/modes/` za specifične načine rada ili `~/.config/legion/modes/` za globalne načine:
