---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Legion konfiguraciji.
...
u `~/.local/share/legion/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Legion
config.
...
Legion Zen je lista modela koje je obezbedio Legion tim koji su bili
testirano i potvrđeno da dobro radi sa Legion. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Legion Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite legion i idite na [legion.dev/auth](https://legion.dev/auth).
...
Radi kao i svaki drugi provajder u Legion i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s Legion:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete legion:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `legion.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Legion.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi legion ispravno radio.
   :::
...
Također možete dodati modele kroz svoju legion konfiguraciju.
...
da pohrani token u legion auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Legion koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Legion
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`legion.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### Konfiguracija

Prilagodite putem `legion.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s legion:
...
Za korištenje Google Vertex AI s Legion:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće legion.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem legiona, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) da automatski prijavite svaki Legion razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Legion razgovor naveden kao zasebna sesija.
...
Možete konfigurirati legion za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati legion za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati legion za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Legion. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/legion) za detalje.
:::
...
Da biste koristili Ollama Cloud s Legion:
...
3. Kopirajte API ključ za korištenje u Legion.
...
6. **Važno**: Prije upotrebe modela oblaka u Legion, morate lokalno povući informacije o modelu:
...
Legion Zen je lista testiranih i verifikovanih modela koju je obezbedio Legion tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Legion Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Legion Zen**.

```txt
   /connect
```

3. Unesite svoj Legion API ključ.
...
Također možete dodati dodatne modele putem vaše legion konfiguracije.
...
5. Također ih možete prilagoditi putem vaše legion konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Legionom:
...
Također možete prilagoditi modele kroz svoju legion konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s legion-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `legion.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Legion da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `legion auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Legion konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj legion konfiguraciji.
