---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Legion.
---

Dodaci vam omogućavaju da proširite Legion spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Legion.
...
- `.legion/plugins/` - Dodaci na nivou projekta
- `~/.config/legion/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/legion/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/legion/legion.json`)
2. Konfiguracija projekta (`legion.json`)
3. Globalni direktorij dodataka (`~/.config/legion/plugins/`)
4. Direktorij dodataka projekta (`.legion/plugins/`)
...
Legion pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Legion SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Legion.
...
:::note
Ako koristite desktop aplikaciju Legion, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite legion da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u legion:
...
Pomoćnik `tool` kreira prilagođeni alat koji legion može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za legion zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://legion.dev/docs/sdk) za detalje.
