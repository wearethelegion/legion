...
Da biste otklonili probleme s Legion, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\legion\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `legion --log-level DEBUG`.

---

## Pohrana

Legion pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/legion/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\legion`
...
Legion Desktop pokreće lokalni Legion server (`legion-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Legion` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/legion/legion.jsonc` (ili `~/.config/legion/legion.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/legion/legion.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\legion\legion.jsonc`
...
Legion također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/legion/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\legion\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Legion mogao ponovo izgraditi.

1. Potpuno zatvorite Legion Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/legion`
- **Linux**: obrišite `~/.cache/legion` (ili pokrenite `rm -rf ~/.cache/legion`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\legion`

3. Ponovo pokrenite Legion Desktop.

---

### Rješavanje problema sa vezom na serveru

Legion Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `legion.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `LEGION_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `LEGION_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Legion Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Legion karakteristikama.

---

### Obavještenja se ne prikazuju

Legion Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Legion u postavkama vašeg OS-a, i
...
1. Zatvorite Legion Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Legion Desktop aplikacije):

- `legion.settings.dat` (URL zadanog servera za desktop)
- `legion.global.dat` i `legion.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `legion models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\legion`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Legion dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\legion`

2. Ponovo pokrenite Legion da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Legion da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Legion će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
