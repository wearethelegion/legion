...
With Legion you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Legion uses our own `legion` theme.
...
Legion comes with several built-in themes.
...
Legion supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/legion/themes/*.json` or `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.legion/themes/*.json`
4. **Current working directory** - Defined in `./.legion/themes/*.json`
