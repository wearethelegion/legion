---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Legion-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/legion/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Legion-konfigurationen.
...
Legion Zen er en liste over modeller leveret af Legion-teamet, som er blevet testet og verificeret til at fungere godt med Legion. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Legion Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg legion og gå til [legion.dev/auth](https://legion.dev/auth).
...
Det fungerer som alle andre udbydere i Legion og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med Legion:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører legion:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `legion.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Legion understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at legion skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem legion-konfigurationen.
...
gemme tokenet i legion auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Legion bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Legion
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`legion.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `legion.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Legion:
...
Sådan bruger du Google Vertex AI med Legion:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem legion, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) plugin for automatisk at logge hver Legion-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Legion-samtale opført som en separat session.
...
Du kan konfigurere legion til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere legion til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere legion til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Legion. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/legion) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Legion:
...
3. Kopier API-nøglen til brug i Legion.
...
6. **Vigtigt**: Før du bruger skymodeller i Legion, skal du hente modelinformationen lokalt:
...
Legion Zen er en liste over testede og verificerede modeller leveret af Legion-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Legion Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Legion Zen**.

   ```txt
   /connect
   ```

3. Indtast Legion API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem legion-konfigurationen.
...
5. Du kan også tilpasse dem gennem legion-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Legion:
...
Du kan også tilpasse modeller gennem legion-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med legion. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `legion.json`-filen i projektkataloget:
...
`limit`-felterne lader Legion forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `legion auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek legion-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i legion-konfigurationen.
