...
Med Legion kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Legion vårt eget `legion`-tema.
...
Legion kommer med flere innebygde temaer.
...
- Vil at Legion skal matche terminalens utseende
...
Legion støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/legion/themes/*.json` eller `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.legion/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.legion/themes/*.json`
