...
For at fejlfinde problemer med Legion, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\legion\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `legion --log-level DEBUG`.

---

## Lagring

legion gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/legion/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\legion`
...
Legion Desktop kører en lokal Legion-server (`legion-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Legion`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/legion/legion.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\legion\legion.jsonc`
...
Legion kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/legion/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\legion\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Legion kan genopbygge den.

1. Afslut Legion Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/legion`
- **Linux**: slet `~/.cache/legion` (eller kør `rm -rf ~/.cache/legion`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\legion`

3. Start Legion Desktop på ny.

---

### Løs problemer med serverforbindelse

Legion Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `legion.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `LEGION_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `LEGION_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Legion Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Legion.

---

### Meddelelser vises ikke

Legion Desktop viser kun systemvarsler når:

- varsler er aktiveret for Legion i dine OS-indstillinger, og
...
1. Afslut Legion Desktop.
2. Find og slet disse filer (de findes i Legion Desktop-appens datamappe):

- `legion.settings.dat` (skrivebordsstandardserver URL)
- `legion.global.dat` og `legion.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Legion:
...
For at finde ud af hvilke modeller du har adgang til, kør `legion models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\legion`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. legion installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\legion`

2. Start legion på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge legion til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
legion vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
