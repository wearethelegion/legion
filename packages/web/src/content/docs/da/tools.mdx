...
Her er alle de innebygde verktøyene som er tilgængelige i Legion.
...
:::note
Dette verktøyet er bare tilgængelig når `LEGION_EXPERIMENTAL_LSP_TOOL=true` (eller `LEGION_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Legion-leverandøren eller når miljøvariabelen `LEGION_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
