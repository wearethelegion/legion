---
title: Konfiguration
description: Ved at bruge Legion JSON konfig.
---

Du kan konfigurere Legion ved hjælp af en JSON-konfigurationsfil.

---

## Format

Legion understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis dine globale konfigurationssæt `theme: "legion"` og `autoupdate: true`, og dine projektkonfigurationssæt `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfiguration integrere alle tre indstillinger.
...
1. **Fjernkonfiguration** (fra `.well-known/legion`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/legion/legion.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`LEGION_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`legion.json` i projekt) - projektspecifikke indstillinger
5. **`.legion` mapper** - agent, kommandoer, plugins
6. **Inline config** (`LEGION_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.legion` og `~/.config/legion` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/legion`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Legion-konfiguration i `~/.config/legion/legion.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `legion.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Legion starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `LEGION_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `LEGION_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.legion` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.legion` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`legion.dev/config.json`**](https://legion.dev/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `legion serve` og `legion web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Legion-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `legion.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Legion-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Legion at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Legion-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/legion/agents/` eller `.legion/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Legion falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`legion run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/legion/commands/` eller `.legion/commands/`. [Learn more here](/docs/commands).
...
Legion vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard legion **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Legion med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.legion/plugins/` eller `~/.config/legion/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Legion til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
