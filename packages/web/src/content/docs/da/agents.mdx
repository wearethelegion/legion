...
Der er to typer agenter i Legion; primære agenter og subagenter.
...
Legion leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Legion leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

Legion leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `legion.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger Legion modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Legion-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Legion-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Legion Zen](/docs/zen), vil du bruge `legion/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `legion models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/wearethelegion/legion).
:::
