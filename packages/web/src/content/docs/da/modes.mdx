...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i legion-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i legion giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem legion-konfigurationen.
...
legion leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.legion/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `legion.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger legion modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale legion-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.legion/modes/` for projektspecifikke tilstande eller `~/.config/legion/modes/` for globale tilstande:
