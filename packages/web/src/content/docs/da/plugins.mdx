---
title: Plugins
description: Skriv dine egne plugins for at udvide Legion.
---

Plugins giver dig mulighed for at udvide Legion ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Legions standardadfærd.
...
- `.legion/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/legion/node_modules/`.
...
1. Global konfiguration (`~/.config/legion/legion.json`)
2. Projektkonfiguration (`legion.json`)
3. Global plugin-mappe (`~/.config/legion/plugins/`)
4. Projekt plugin bibliotek (`.legion/plugins/`)
...
Legion kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En legion SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide legion.
...
:::note
Hvis du bruger Legion desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå legion i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til legion:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som legion kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for legion sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://legion.dev/docs/sdk) for detaljer.
