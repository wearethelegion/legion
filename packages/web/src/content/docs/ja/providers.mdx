---
title: プロバイダー
description: Legion で LLM プロバイダーを使用する。
---

import config from "../../../../config.mjs"
export const console = config.console

Legion は [AI SDK](https://ai-sdk.dev/) および [Models.dev](https://models.dev) を使用して **75 以上の LLM プロバイダー** をサポートし、ローカルモデルの実行をサポートします。

プロバイダーを追加するには、次のことを行う必要があります。

1. `/connect` コマンドを使用して、プロバイダーの API キーを追加します。
2. Legion 設定でプロバイダーを構成します。
...
`~/.local/share/legion/auth.json`で。

---

### 設定

Legion の `provider` セクションを通じてプロバイダーをカスタマイズできます。
config.
...
Legion Zen は、Legion チームが提供するモデルのリストです。
Legion で適切に動作することがテストおよび検証されています。 [詳細はこちら](/docs/zen)。

:::tip
初めての方は、Legion Zen から始めることをお勧めします。
:::

1. TUI で `/connect` コマンドを実行し、legion を選択して、[legion.dev/auth](https://legion.dev/auth) で認証します。
...
これは Legion の他のプロバイダーと同様に機能し、使用は完全にオプションです。
...
Legion で Amazon Bedrock を使用するには:
...
プロジェクト固有の設定または永続的な設定の場合は、`legion.json` を使用します。
...
:::info
Legion での Claude Pro/Max サブスクリプションの使用は、[Anthropic](https://anthropic.com) によって正式に許可されていませんが、一般的に機能します。
:::
...
:::note
Legion が正しく動作するには、デプロイメント名がモデル名と一致する必要があります。
:::
...
トークンを Legion 認証ストレージに保存します。
:::

##### 自己ホスト型 GitLab

:::note[コンプライアンスメモ]
Legion は、セッションタイトルの生成などの一部の AI タスクに小規模なモデルを使用します。
デフォルトでは、Zen によってホストされる gpt-5-nano を使用するように構成されています。 Legion をロックするには
自分の GitLab でホストされているインスタンスのみを使用するには、次の行を
`legion.json` ファイル。セッション共有を無効にすることもお勧めします。
...
詳細については、[legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth) ホームページ。

##### 設定

`legion.json` を通じてカスタマイズします。
...
GitHub Copilot サブスクリプションを Legion で使用するには:
...
Legion で Google Vertex AI を使用するには:
...
Legion を通じて自動的に構成されていない Helicone の機能またはモデルを見つけた場合は、いつでも自分で構成できます。
...
Helicone の [Sessions](https://docs.helicone.ai/features/sessions) 機能を使用すると、関連する LLM リクエストをグループ化できます。 [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) プラグインを使用して、各 Legion 会話を Helicone のセッションとして自動的に記録します。
...
プラグインは、リクエストに `Helicone-Session-Id` ヘッダーと `Helicone-Session-Name` ヘッダーを挿入します。 Helicone のセッションページでは、Legion の各会話が個別のセッションとしてリストされています。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように legion を構成できます。
...
LM Studio を通じてローカルモデルを使用するように legion を構成できます。
...
Ollama を通じてローカルモデルを使用するように legion を構成できます。

:::tip
Ollama は Legion 用に自動的に構成できます。詳細については、[Ollama 統合 docs](https://docs.ollama.com/integrations/legion)」を参照してください。
:::
...
6. **重要**: Legion でクラウドモデルを使用する前に、モデル情報をローカルに取得する必要があります。
...
Legion Zen は、Legion チームによって提供される、テストおよび検証されたモデルのリストです。 [詳細はこちら](/docs/zen)。

1. **<a href={console}>Legion Zen</a>** にサインインし、**API キーの作成** をクリックします。

2. `/connect` コマンドを実行し、**Legion Zen** を検索します。

   ```txt
   /connect
   ```

3. Legion API キーを入力します。
...
Legion 設定を通じて追加のモデルを追加することもできます。
...
5. Legion 設定を通じてカスタマイズすることもできます。プロバイダーを指定する例を次に示します。
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) を Legion で使用するには:
...
Legion 設定を通じてモデルをカスタマイズすることもできます。プロバイダーのルーティング順序を指定する例を次に示します。
...
:::tip
OpenAI と互換性のある任意のプロバイダーを Legion で使用できます。最新の AI プロバイダーのほとんどは、OpenAI 互換の API を提供しています。
:::
...
4. プロジェクトディレクトリで `legion.json` ファイルを作成または更新します。
...
`limit` フィールドを使用すると、Legion はコンテキストがどのくらい残っているかを理解できます。標準プロバイダーは、これらを models.dev から自動的に取得します。
...
1. **認証設定を確認します**: `legion auth list` を実行して、資格情報が正しいかどうかを確認します。
   プロバイダー用のものが設定に追加されます。

これは、認証に環境変数に依存する Amazon Bedrock のようなプロバイダーには当てはまりません。

2. カスタムプロバイダーの場合は、Legion 設定を確認し、次のことを行います。
   - `/connect` コマンドで使用されるプロバイダー ID が、Legion 設定内の ID と一致することを確認してください。
