---
title: 設定
description: Legion JSON 設定を使用します。
---

JSON 設定ファイルを使用して Legion を構成できます。

---

## 形式

Legion は、**JSON** と **JSONC** (コメント付きの JSON) 形式の両方をサポートしています。
...
たとえば、グローバル設定で `theme: "legion"` と `autoupdate: true` が設定され、プロジェクト設定で `model: "anthropic/claude-sonnet-4-5"` が設定されている場合、最終的な設定には 3 つの設定がすべて含まれます。
...
1. **リモート設定** (`.well-known/legion` から) - 組織のデフォルト
2. **グローバル設定** (`~/.config/legion/legion.json`) - ユーザー設定
3. **カスタム設定** (`LEGION_CONFIG` 環境変数) - カスタムオーバーライド
4. **プロジェクト設定** (プロジェクト内の `legion.json`) - プロジェクト固有の設定
5. **`.legion` ディレクトリ** - エージェント、コマンド、プラグイン
6. **インライン設定** (`LEGION_CONFIG_CONTENT` 環境変数) - ランタイムオーバーライド

つまり、プロジェクト設定はグローバルのデフォルトをオーバーライドでき、グローバル設定はリモート組織のデフォルトをオーバーライドできます。

:::note
`.legion` および `~/.config/legion` ディレクトリでは、サブディレクトリに **複数名** が使用されています: `agents/`、`commands/`、`modes/`、`plugins/`、`skills/`、`tools/`、および `themes/`。下位互換性のために、単数形の名前 (`agent/` など) もサポートされています。
:::
...
組織は、`.well-known/legion` エンドポイント経由でデフォルト設定を提供できます。これは、それをサポートするプロバイダーで認証するときに自動的に取得されます。
...
グローバル Legion 設定を `~/.config/legion/legion.json` に配置します。テーマ、プロバイダー、キーバインドなどのユーザー全体の設定にはグローバル設定を使用します。
...
プロジェクトのルートに `legion.json` を追加します。プロジェクト設定は、標準設定ファイルの中で最も高い優先順位を持ち、グローバル設定とリモート設定の両方をオーバーライドします。

:::tip
プロジェクト固有の設定をプロジェクトのルートに配置します。
:::

Legion が起動すると、現在のディレクトリで設定ファイルを検索するか、最も近い Git ディレクトリまで移動します。
...
`LEGION_CONFIG` 環境変数を使用してカスタム設定ファイルのパスを指定します。
...
`LEGION_CONFIG_DIR` を使用してカスタム設定ディレクトリを指定します。
環境変数。このディレクトリでは、エージェント、コマンド、
モードとプラグインは標準の `.legion` ディレクトリと同様であり、次のようにする必要があります。
同じ構造に従います。
...
カスタムディレクトリはグローバル config ディレクトリと `.legion` ディレクトリの後にロードされるため、それらの設定を**オーバーライド**できます。

---

## スキーマ

設定ファイルには、[**`legion.dev/config.json`**](https://legion.dev/config.json) を使用します。
...
`legion serve` オプションを使用して、`legion web` および `server` コマンドのサーバー設定を構成できます。
...
- `mdns` - mDNS サービス検出を有効にします。これにより、ネットワーク上の他のデバイスが Legion サーバーを検出できるようになります。
- `mdnsDomain` - mDNS サービスのカスタムドメイン名。デフォルトは `legion.local` です。同じネットワーク上で複数のインスタンスを実行する場合に便利です。
...
`provider`、`model`、および `small_model` オプションを使用して、Legion 設定で使用するプロバイダーとモデルを構成できます。
...
`small_model` オプションは、タイトル生成などの軽量タスク用に別のモデルを構成します。デフォルトでは、Legion は、プロバイダーから安価なモデルが入手可能な場合は、より安価なモデルを使用しようとします。そうでない場合は、メインモデルにフォールバックします。
...
`theme` オプションを使用して、Legion 設定で使用するテーマを構成できます。
...
`~/.config/legion/agents/` または `.legion/agents/` の Markdown ファイルを使用してエージェントを定義することもできます。 [詳細はこちら](/docs/agents)。
...
デフォルトのエージェントはプライマリエージェントである必要があります (サブエージェントではありません)。これは、`"build"` や `"plan"` のような組み込みエージェント、または定義した[カスタムエージェント](/docs/agents) にすることができます。指定されたエージェントが存在しないか、サブエージェントである場合、Legion は警告とともに `"build"` にフォールバックします。

この設定は、TUI、CLI (`legion run`)、デスクトップアプリ、および GitHub Action のすべてのインターフェイスに適用されます。
...
`~/.config/legion/commands/` または `.legion/commands/` の Markdown ファイルを使用してコマンドを定義することもできます。 [詳細はこちら](/docs/commands)。
...
Legion は起動時に新しいアップデートを自動的にダウンロードします。 `autoupdate` オプションを使用してこれを無効にできます。
...
デフォルトでは、legion は明示的な承認を必要とせずに **すべての操作を許可**します。これは、`permission` オプションを使用して変更できます。
...
[Plugins](/docs/plugins) は、カスタムツール、フック、統合を使用して Legion を拡張します。

プラグインファイルを `.legion/plugins/` または `~/.config/legion/plugins/` に配置します。 `plugin` オプションを使用して npm からプラグインをロードすることもできます。
...
これは、Legion を 1 つずつ無効にするのではなく、特定のプロバイダーのみを使用するように制限したい場合に便利です。
