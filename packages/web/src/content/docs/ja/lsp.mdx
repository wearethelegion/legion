---
title: LSPサーバー
description: Legion は LSP サーバーと統合します。
---

Legion は言語サーバープロトコル (LSP) と統合して、LLM がコードベースと対話できるようにします。診断を使用して LLM にフィードバックを提供します。

---

## 組み込み

Legion には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
:::note
`LEGION_DISABLE_LSP_DOWNLOAD` 環境変数を `true` に設定すると、LSP サーバーの自動ダウンロードを無効にできます。
:::
...
Legion 設定の `lsp` セクションを通じて LSP サーバーをカスタマイズできます。
