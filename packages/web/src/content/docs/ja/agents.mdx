...
Legion には 2 種類のエージェントがあります。プライマリエージェントとサブエージェント。
...
Legion には、**Build** と **Plan** という 2 つの組み込みプライマリエージェントが付属しています。これらについて以下で説明します。
...
Legion には、**General** と **Explore** という 2 つの組み込みサブエージェントが付属しています。これについては以下で見ていきます。

---

## 組み込み

Legion には、2 つの組み込みプライマリエージェントと 2 つの組み込みサブエージェントが付属しています。
...
`legion.json` 設定ファイルでエージェントを構成します。
...
- グローバル: `~/.config/legion/agents/`
- プロジェクトごと: `.legion/agents/`
...
温度が指定されていない場合、Legion はモデル固有のデフォルトを使用します。通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55 です。
...
このパスは、設定ファイルが配置されている場所に対する相対パスです。したがって、これはグローバルな Legion 設定とプロジェクト固有の設定の両方で機能します。
...
Legion 設定内のモデル ID は、`provider/model-id` という形式を使用します。たとえば、[Legion Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `legion/gpt-5.1-codex` を使用します。
...
:::tip
`legion models` を実行して、利用可能なモデルのリストを表示します。
:::
...
:::tip
共有したいエージェントはいますか? [PR](https://github.com/wearethelegion/legion) を送信します。
:::
