...
legion를 사용하면 여러 내장 테마 중 하나에서 선택할 수 있으며 terminal 테마에 적응하는 테마를 사용하거나 사용자 정의 테마를 정의 할 수 있습니다.

기본적으로 legion는 자체 `legion` 테마를 사용합니다.
...
- legion가 terminal의 외관과 일치해야 합니다.
...
legion는 사용자가 쉽게 테마를 만들 수 있도록 유연한 JSON 기반 테마 시스템을 지원합니다.
...
1.**Built-in themes** - 이것은 바이너리에 내장되어 있습니다. 2. **사용자 설정 디렉토리 ** - `~/.config/legion/themes/*.json` 또는 `$XDG_CONFIG_HOME/legion/themes/*.json`에서 정의 3. ** 루트 디렉토리 ** - `<project-root>/.legion/themes/*.json`에서 정의 4. **현재 작업 디렉토리 ** - `./.legion/themes/*.json`에서 정의
