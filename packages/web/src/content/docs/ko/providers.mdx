---
title: 공급자
description: Legion에서 LLM 공급자를 사용합니다.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion는 [AI SDK](https://ai-sdk.dev/) 및 [Models.dev](https://models.dev)를 사용하여 **75개 이상의 LLM 공급자**를 지원하며 로컬 모델도 실행할 수 있습니다.

필요한 공급자를 추가하려면:

1. `/connect` 명령을 사용하여 공급자의 API 키를 추가하십시오.
2. Legion 구성에서 공급자를 구성하십시오.
...
Legion 설정의 `provider` 섹션을 통해 공급자를 사용자 정의할 수 있습니다.
...
Legion Zen은 Legion 팀이 Legion와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).

:::tip
처음이라면 Legion Zen으로 시작하는 것이 좋습니다.
:::

1. TUI에서 `/connect` 명령을 실행하고 `legion`를 선택한 뒤, [legion.dev/auth](https://legion.dev/auth)로 이동합니다.
...
Legion의 다른 공급자처럼 작동하며 사용은 완전히 선택 사항입니다.
...
Legion를 실행하는 동안 다음 환경 변수 중 하나를 설정합니다:
...
프로젝트별 또는 영구 구성을 위해 `legion.json`을 사용하십시오.
...
:::info
Legion에서 Claude Pro/Max 구독을 사용하는 것은 [Anthropic](https://anthropic.com)에서 공식적으로 지원하지 않습니다.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)로 이동하여 모델을 배포합니다.
   ...
   :::note
   배포 이름은 제대로 작동하려면 Legion의 모델 이름과 일치해야 합니다.
   :::
...
Legion는 인증 저장소에 토큰을 저장합니다.
:::

#### 셀프 호스팅 GitLab (Self-Hosted)

:::note[규정 준수 참고 사항]
Legion는 세션 제목 생성과 같은 일부 AI 작업을 위해 작은 모델을 사용합니다.
기본적으로 Zen에서 호스팅되는 gpt-5-nano를 사용하도록 구성됩니다.
Legion를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `legion.json` 파일에 다음을 추가하십시오.
...
[legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth) 홈페이지에 추가 문서가 있습니다.

##### 구성

`legion.json`을 통해 사용자 정의:
...
GitHub Copilot 구독을 사용하여 legion:
...
legion로 Google Vertex AI를 사용하려면:
...
이벤트에서 legion를 통해 자동으로 구성되지 않는 Helicone의 기능 또는 모델을 볼 수 있습니다.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) 기능으로 그룹 관련 LLM 요청이 가능합니다. [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) 플러그인을 사용하여 각 legion 대화를 Helicone 세션으로 자동 로그인하십시오.
...
플러그인은 `Helicone-Session-Id` 및 `Helicone-Session-Name` 헤더를 귀하의 요청에 주사합니다. Helicone의 세션 페이지에서는 별도의 세션으로 나열된 각 legion 대화를 볼 수 있습니다.
...
:::tip
Ollama는 legion를 자동으로 구성할 수 있습니다. 자세한 내용은 [Ollama 통합 문서](https://docs.ollama.com/integrations/legion)를 참조하십시오.
:::
...
3. legion에서 사용을 위한 API 열쇠를 복사하십시오.
...
6. ** 중요 **: legion의 클라우드 모델을 사용하기 전에, 로컬 모델 정보를 끌어야 합니다:
...
Legion Zen은 legion 팀에서 제공하는 테스트 및 검증된 모델 목록입니다. [더 알아보기](/docs/zen).

1. 로그인 **<a href={console}>Legion Zen</a>** and click**Create API Key**.

2. `/connect` 명령을 실행하고 **Legion Zen**를 검색하십시오.

   ```txt
   /connect
   ```

3. legion API 키를 입력하십시오.
...
legion config를 통해 추가 모델을 추가할 수 있습니다.
...
5. 당신은 또한 당신의 legion config를 통해 그들을 주문을 받아서 만들 수 있습니다. 공급자 지정의 예입니다.
...
legion를 사용하여 [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)를 사용하려면:
...
legion config를 통해 모델을 사용자 정의 할 수 있습니다. 공급자 routing 순서를 지정하는 예입니다.
...
:::tip
legion를 사용하여 OpenAI 호환 공급자를 사용할 수 있습니다. 가장 현대적인 AI 제공 업체는 OpenAI 호환 API를 제공합니다.
:::
...
4. 프로젝트 디렉토리에 `legion.json` 파일을 만들거나 업데이트하십시오.
...
`limit` 필드를 사용하면 legion가 얼마나 많은 컨텍스트를 이해 할 수 있습니다. 표준 공급자는 model.dev에서 자동적으로 당깁니다.
...
1. **주의 설정 확인 **: `legion auth list`를 실행하여 자격 증명을 볼 수 있습니다.
   공급자는 config에 추가됩니다.

이것은 Amazon Bedrock과 같은 공급자에 적용되지 않습니다. 환경 변수에 의존합니다.

2. 주문 공급자를 위해, legion config를 검사하고:

- `/connect` 명령에 사용되는 공급자 ID가 legion config에서 ID를 일치시킵니다.
