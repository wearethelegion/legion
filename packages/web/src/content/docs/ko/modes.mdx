...
:::caution
모드는 legion 설정에서 `agent` 옵션을 통해 구성되어 있습니다. 더 보기
`mode` 옵션이 이제 비활성화되었습니다. [더 알아보기](/docs/agents).
:::

legion의 모드는 다른 사용 사례에 대한 행동, 도구 및 프롬프트를 사용자 정의 할 수 있습니다.

그것은 두 개의 내장 모드와 함께 제공됩니다 : ** 빌드 ** 및 ** 계획 **. 사용자 정의 할 수 있습니다.
legion config를 통해 자체를 구성합니다.
...
- `edit` - `.legion/plans/*.md`에 위치한 파일을 제외하고 기존 파일을 수정할 수 없습니다.
- `patch` - 패치 적용
...
온도가 지정되지 않은 경우, legion는 모델별 기본 (일반적으로 0 대부분의 모델에 대한, 0.55 Qwen 모델)을 사용합니다.
...
글로벌 legion config 및 프로젝트 특정 구성 모두.
...
프로젝트 별 모드 또는 `~/.config/legion/modes/`의 모드 파일을 만들 수 있습니다.
