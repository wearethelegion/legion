---
title: LSP 서버
description: Legion는 LSP 서버와 통합되어 작동합니다.
---

Legion는 언어 서버 프로토콜(LSP)과 통합되어 LLM이 코드베이스와 상호 작용할 수 있게 돕습니다. 이를 통해 진단 정보를 사용하여 LLM에 피드백을 제공합니다.

---

## 기본 제공 (Built-in)

Legion는 인기 있는 언어들에 대해 여러 내장 LSP 서버를 제공합니다.
...
:::note
`LEGION_DISABLE_LSP_DOWNLOAD` 환경 변수를 `true`로 설정하여 자동 LSP 서버 다운로드를 비활성화 할 수 있습니다.
:::
...
Legion 설정의 `lsp` 섹션을 통해 LSP 서버를 사용자 정의할 수 있습니다.
