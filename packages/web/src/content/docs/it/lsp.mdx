---
title: Server LSP
description: Legion si integra con i tuoi server LSP.
---

Legion si integra con il tuo Language Server Protocol (LSP) per aiutare l'LLM a interagire con la tua codebase. Usa le diagnostiche per fornire feedback all'LLM.

---

## Integrati

Legion include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `LEGION_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Puoi personalizzare i server LSP tramite la sezione `lsp` nella configurazione di legion.
