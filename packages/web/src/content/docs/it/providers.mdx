---
title: Provider
description: Usa qualsiasi provider LLM in Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Legion.
...
Legion Zen e una lista di modelli forniti dal team Legion, testati e
verificati per funzionare bene con Legion. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Legion Zen.
:::

1. Esegui `/connect` nella TUI, seleziona legion e vai su [legion.dev/auth](https://legion.dev/auth).
...
Funziona come qualsiasi altro provider in Legion ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Legion:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   Imposta una di queste variabili d'ambiente mentre esegui legion:
   ...
   Per configurazione specifica del progetto o persistente, usa `legion.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Legion non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché legion funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di legion.
...
salvare il token nello storage di auth di legion.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Legion usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Legion
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `legion.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### Configurazione

Personalizza tramite `legion.json`:
...
Per usare il tuo abbonamento GitHub Copilot con legion:
...
Per usare Google Vertex AI con Legion:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui legion.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da legion, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) per loggare automaticamente ogni conversazione di Legion come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Legion elencata come sessione separata.
...
Puoi configurare legion per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare legion per usare modelli locali tramite LM Studio.
...
Puoi configurare legion per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Legion. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/legion) per dettagli.
:::
...
Per usare Ollama Cloud con Legion:
...
3. Copia la chiave API da usare in Legion.
...
6. **Importante**: Prima di usare modelli cloud in Legion, devi scaricare le informazioni del modello localmente:
...
Legion Zen è una lista di modelli testati e verificati forniti dal team Legion. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Legion Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Legion Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Legion.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di legion.
...
5. Puoi anche personalizzarli tramite la configurazione di legion. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Legion:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di legion. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con legion. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `legion.json` nella directory del progetto:
...
I campi `limit` permettono a Legion di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `legion auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di legion e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config legion.
