...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Legion include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Legion.
...
:::note
Questo strumento e' disponibile solo quando `LEGION_EXPERIMENTAL_LSP_TOOL=true` (oppure `LEGION_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Legion o quando la variabile d'ambiente `LEGION_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Legion:
