...
Puoi aggiungere strumenti esterni a Legion usando il _Model Context Protocol_, o MCP. Legion supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Legion Config](https://legion.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/legion`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Legion gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Legion:
...
Se il server richiede autenticazione, Legion ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `legion mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Legion memorizzera' i token in modo sicuro in `~/.local/share/legion/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Legion insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Legion come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Legion al tuo account Sentry.
