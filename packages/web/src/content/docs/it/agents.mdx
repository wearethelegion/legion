...
In Legion esistono due tipi di agenti: agenti primari e subagenti.
...
Legion include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Legion include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Legion include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `legion.json`:
...
- Globale: `~/.config/legion/agents/`
- Per progetto: `.legion/agents/`
...
Se non specifichi una temperature, Legion usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Legion sia per la config specifica del progetto.
...
L'ID modello nella configurazione Legion usa il formato `provider/model-id`. Per esempio, se stai usando [Legion Zen](/docs/zen), useresti `legion/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `legion models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/wearethelegion/legion).
:::
