---
title: Plugin
description: Scrivi plugin per estendere Legion.
---

I plugin ti permettono di estendere Legion agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di Legion.
...
- `.legion/plugins/` - plugin a livello progetto
- `~/.config/legion/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/legion/node_modules/`.
...
1. Config globale (`~/.config/legion/legion.json`)
2. Config di progetto (`legion.json`)
3. Directory plugin globale (`~/.config/legion/plugins/`)
4. Directory plugin di progetto (`.legion/plugins/`)
...
Legion esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di legion per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere legion.
...
:::note
Se usi l'app desktop di Legion, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a legion di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a legion:
...
L'helper `tool` crea uno strumento personalizzato che legion puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in legion insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://legion.dev/docs/sdk) per i dettagli.
