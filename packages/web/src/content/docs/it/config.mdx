...
description: Usa la configurazione JSON di Legion.
---

Puoi configurare Legion usando un file di configurazione JSON.

---

## Formato

Legion supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Per esempio, se la tua configurazione globale imposta `theme: "legion"` e `autoupdate: true`, e la configurazione del progetto imposta `model: "anthropic/claude-sonnet-4-5"`, la configurazione finale includera tutte e tre le impostazioni.
...
1. **Config remota** (da `.well-known/legion`) - default dell'organizzazione
2. **Config globale** (`~/.config/legion/legion.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `LEGION_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`legion.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.legion`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `LEGION_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.legion` e `~/.config/legion` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/legion`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Legion in `~/.config/legion/legion.json`. Usa la configurazione globale per preferenze valide per l'utente (ad es. temi, provider o keybind).
...
Aggiungi `legion.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.

:::tip
Metti la configurazione specifica del progetto nella root del progetto.
:::

Quando Legion si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `LEGION_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `LEGION_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.legion` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.legion`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`legion.dev/config.json`**](https://legion.dev/config.json).
...
Puoi configurare le impostazioni del server per i comandi `legion serve` e `legion web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Legion.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `legion.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Legion tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Legion prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi configurare il tema da usare in Legion tramite l'opzione `theme`.
...
Puoi anche definire agenti usando file markdown in `~/.config/legion/agents/` o `.legion/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Legion fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`legion run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/legion/commands/` o `.legion/commands/`. [Scopri di piu](/docs/commands).
...
Legion scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, legion **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Legion con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.legion/plugins/` o `~/.config/legion/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Legion a usare solo alcuni provider invece di disabilitarli uno per uno.
