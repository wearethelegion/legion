---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Legion.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Legion fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Legion avvia la TUI per la directory corrente.
...
Quando usi la TUI di Legion, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Legion. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Puoi personalizzare il comportamento della TUI tramite il file di config di Legion.
