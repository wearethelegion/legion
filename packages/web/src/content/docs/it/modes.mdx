...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di legion. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in legion ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di legion.
...
legion include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.legion/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `legion.json`:
...
- Globale: `~/.config/legion/modes/`
- Progetto: `.legion/modes/`
...
Se non viene specificata alcuna temperatura, legion usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di legion sia per quella specifica del progetto.
...
Crea file di modalita in `.legion/modes/` per modalita specifiche del progetto oppure in `~/.config/legion/modes/` per modalita globali:
