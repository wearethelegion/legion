...
Wenn Legion Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `legion --log-level DEBUG`.

---

## Speicher

legion speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
Legion Desktop startet im Hintergrund einen lokalen Legion-Server (`legion-cli`-Sidecar).
...
- Nur macOS: `Legion`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit Legion hast:
