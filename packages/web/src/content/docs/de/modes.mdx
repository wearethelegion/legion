...
:::caution
Modi werden jetzt über die Option `agent` in der Legion-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in Legion können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die Legion-Konfiguration.
...
Legion verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.legion/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `legion.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Legion modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale Legion-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.legion/modes/` für projektspezifische Modi oder `~/.config/legion/modes/` für globale Modi:
