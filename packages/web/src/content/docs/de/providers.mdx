---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Legion-Konfiguration.
...
in `~/.local/share/legion/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem Legion anpassen
config.
...
Legion Zen ist eine Liste von Modellen, die vom Legion-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Legion funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Legion Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Legion aus und gehen Sie zu [legion.dev/auth](https://legion.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in Legion und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit Legion:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Legion ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `legion.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Legion wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Legion ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Legion-Konfiguration hinzufügen.
...
um das Token im Legion-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Legion verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Legion
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`legion.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `legion.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Legion:
...
So verwenden Sie Google Vertex AI mit Legion:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Legion ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Legion konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session)-Plugin, um jede Legion-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Legion-Konversation als separate Sitzung aufgeführt.
...
Sie können Legion für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Legion über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Legion über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Legion konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/legion).
:::
...
So verwenden Sie Ollama Cloud mit Legion:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Legion.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Legion verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Legion Zen ist eine Liste getesteter und verifizierter Modelle, die vom Legion-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Legion Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Legion Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Legion API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Legion-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Legion-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Legion:
...
Sie können Modelle auch über Ihre Legion-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Legion verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `legion.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Legion erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `legion auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Legion-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Legion-Konfiguration übereinstimmt.
