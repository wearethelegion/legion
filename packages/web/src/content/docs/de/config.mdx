---
title: Konfiguration
description: Verwenden Sie die Konfiguration Legion JSON.
---

Sie können Legion mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

Legion unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Wenn Ihre globale Konfiguration beispielsweise `theme: "legion"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/legion`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/legion/legion.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`LEGION_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`legion.json` im Projekt) – projektspezifische Einstellungen
5. **`.legion` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`LEGION_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.legion` und `~/.config/legion` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/legion` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale Legion-Konfiguration in `~/.config/legion/legion.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `legion.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn Legion gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `LEGION_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `LEGION_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.legion` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.legion` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`legion.dev/config.json`**](https://legion.dev/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `legion serve` und `legion web` über die Option `server` konfigurieren.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres Legion-Servers erkannt werden.
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `legion.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer Legion-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht Legion, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer Legion-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/legion/agents/` oder `.legion/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift Legion mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`legion run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/legion/commands/` oder `.legion/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Legion lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
Legion erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert Legion mit benutzerdefinierten Tools, Hooks und Integrationen.

Platzieren Sie Ihre Plugin-Dateien in `.legion/plugins/` oder `~/.config/legion/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
...
Dies ist nützlich, wenn Sie Legion darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
