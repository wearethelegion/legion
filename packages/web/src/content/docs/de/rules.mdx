---
title: Regeln
description: Lege eigene Anweisungen fuer legion fest.
---

Du kannst legion ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in legion den Befehl `/init` aus.
...
So kann legion sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht legion sie zu erweitern.
...
legion kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/legion/AGENTS.md` speichern.
Diese gelten in allen legion-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Legion dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/legion/AGENTS.md` existiert)
...
Beim Start sucht legion in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/legion/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `legion.json` oder in `~/.config/legion/legion.json` angeben.
...
legion parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit legion.json

Empfohlen ist das `instructions`-Feld in der `legion.json`:
...
Du kannst legion explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `legion.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
