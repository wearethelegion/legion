---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Legion zu erweitern.
---

Mit Plugins können Sie Legion erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Legion zu ändern.
...
- `.legion/plugins/` – Plugins auf Projektebene
- `~/.config/legion/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/legion/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/legion/legion.json`)
2. Projektkonfiguration (`legion.json`)
3. Globales Plugin-Verzeichnis (`~/.config/legion/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.legion/plugins/`)
...
Legion führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Legion-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Legion erweitern können.
...
:::note
Wenn Sie die Legion-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Legion `.env`-Dateien liest:
...
Plugins können Legion auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Legion aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Legion zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://legion.dev/docs/sdk).
