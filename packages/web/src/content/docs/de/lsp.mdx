---
title: LSP Server
description: Legion lässt sich in Ihren LSP-Server integrieren.
---

Legion lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

Legion verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `LEGION_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn Legion eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer Legion-Konfiguration anpassen.
