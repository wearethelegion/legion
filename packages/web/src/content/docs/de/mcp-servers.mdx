...
Sie können externe Tools zu Legion hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Legion unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Legion Config](https://legion.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/legion`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Legion übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Legion Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Legion beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `legion mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Legion die Token sicher in `~/.local/share/legion/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Legion verfügbar. Sie können sie also wie jedes andere Tool über die Legion-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Legion mit Ihrem Sentry-Konto zu verbinden.
