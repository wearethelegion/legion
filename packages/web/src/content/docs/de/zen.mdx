---
title: Zen
description: Kuratierte Modellliste direkt von Legion.
---
...
Legion Zen ist eine vom Legion-Team getestete und verifizierte Modellliste.

:::note
Legion Zen befindet sich aktuell in der Beta.
:::

Zen funktioniert wie jeder andere Provider in Legion.
Du meldest dich bei Legion Zen an, holst dir deinen API-Key und nutzt ihn optional.
...
:::tip
Wir haben eine Auswahl aus Modellen und Providern getestet, die gut mit Legion funktionieren.
:::
...
Legion Zen ist ein AI-Gateway, das dir Zugriff auf genau diese Modelle gibt.

---

## Funktionsweise

Legion Zen funktioniert wie jeder andere Provider in Legion.

1. Melde dich bei **<a href={console}>Legion Zen</a>** an, hinterlege Zahlungsdaten und kopiere deinen API-Key.
2. Fuehre in der TUI `/connect` aus, waehle Legion Zen und fuege den API-Key ein.
...
Die [Model-ID](/docs/config/#models) in deiner Legion-Konfiguration hat das Format `legion/<model-id>`.
Fuer GPT 5.2 Codex verwendest du zum Beispiel `legion/gpt-5.2-codex`.
...
Wir haben Legion Zen entwickelt, um:
...
4. **Kein Lock-in** zu erzwingen, damit du Zen mit anderen Coding-Agents und Legion weiter mit anderen Providern nutzen kannst
