---
title: LSP Servers
description: Legion integrates with your LSP servers.
---

Legion integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

Legion comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `LEGION_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
You can customize LSP servers through the `lsp` section in your legion config.
