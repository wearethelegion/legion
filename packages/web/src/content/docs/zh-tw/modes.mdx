...
:::caution
模式現在透過 legion 設定中的 `agent` 選項進行設定。`mode` 選項已廢棄。[了解更多](/docs/agents)。
:::

legion 中的模式允許您為不同的使用情境自訂行為、工具和提示詞。

legion 自帶兩種內建模式：**build** 和 **plan**。您可以自訂這些模式，也可以透過 legion 設定建立自己的模式。
...
- `write` - 無法建立新檔案
- `edit` - 無法修改現有檔案，但位於 `.legion/plans/*.md` 的檔案除外，用於詳細說明計畫本身
- `patch` - 無法套用補丁
- `bash` - 無法執行 shell 指令
...
此路徑相對於設定檔所在位置。因此，全域 legion 設定和專案特定設定均可使用。
...
在 `.legion/modes/` 中建立專案特定的模式檔案，或在 `~/.config/legion/modes/` 中建立全域模式檔案：
