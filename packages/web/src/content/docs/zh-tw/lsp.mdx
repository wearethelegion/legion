---
title: LSP 伺服器
description: Legion 與您的 LSP 伺服器整合。
---

Legion 與您的語言伺服器協定（LSP）整合，協助 LLM 與您的程式碼庫進行互動。它利用診斷資訊向 LLM 提供回饋。

---

## 內建支援

Legion 內建了多種適用於主流語言的 LSP 伺服器：
...
:::note
您可以將 `LEGION_DISABLE_LSP_DOWNLOAD` 環境變數設定為 `true` 來停用 LSP 伺服器的自動下載。
:::
...
您可以透過 Legion 設定檔中的 `lsp` 部分來自訂 LSP 伺服器。
