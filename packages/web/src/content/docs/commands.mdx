...
Create `.legion/commands/test.md`:
...
You can add custom commands through the Legion config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Legion [config](/docs/config):
...
legion includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
