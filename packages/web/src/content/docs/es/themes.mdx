...
Con Legion puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Legion usa nuestro propio tema `legion`.
...
Legion viene con varios temas integrados.
...
- Quiere que Legion coincida con la apariencia de su terminal
...
Legion admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/legion/themes/*.json` o `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.legion/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.legion/themes/*.json`
