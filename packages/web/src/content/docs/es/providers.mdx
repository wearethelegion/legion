---
title: Proveedores
description: Usando cualquier proveedor LLM en Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Legion.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Legion
configuración.
...
Legion Zen es una lista de modelos proporcionados por el equipo Legion que han sido
probado y verificado para funcionar bien con Legion. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Legion Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione legion y diríjase a [legion.dev/auth](https://legion.dev/auth).
...
Funciona como cualquier otro proveedor en Legion y su uso es completamente opcional.
...
Para usar Amazon Bedrock con Legion:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta legion:
   ...
   Para una configuración persistente o específica del proyecto, utilice `legion.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Legion no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que legion funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración legion.
...
para almacenar el token en el almacenamiento de autenticación legion.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Legion utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Legion
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `legion.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### Configuración

Personalizar a través de `legion.json`:
...
Para utilizar su suscripción GitHub Copilot con legion:
...
Para utilizar Google Vertex AI con Legion:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta legion.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de legion, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) para registrar automáticamente cada conversación Legion como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Legion enumerada como una sesión separada.
...
Puede configurar legion para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar legion para usar modelos locales a través de LM Studio.
...
Puedes configurar legion para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Legion. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/legion) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Legion:
...
3. Copie la clave API para usarla en Legion.
...
6. **Importante**: Antes de usar modelos en la nube en Legion, debe obtener la información del modelo localmente:
...
Legion Zen es una lista de modelos probados y verificados proporcionada por el equipo Legion. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Legion Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Legion Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Legion API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración legion.
...
5. También puedes personalizarlos a través de tu configuración legion. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Legion:
...
También puede personalizar modelos a través de su configuración legion. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con legion. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `legion.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Legion comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `legion auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración legion y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración legion.
