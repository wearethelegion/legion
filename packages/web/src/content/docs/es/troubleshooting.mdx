...
Para depurar problemas con Legion, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\legion\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `legion --log-level DEBUG`.

---

## Almacenamiento

legion almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/legion/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\legion`
...
Legion Desktop ejecuta un servidor Legion local (el sidecar `legion-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Legion` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/legion/legion.jsonc` (o `~/.config/legion/legion.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/legion/legion.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\legion\legion.jsonc`
...
Legion también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/legion/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\legion\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Legion pueda reconstruirlo.

1. Salga de Legion Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/legion`
- **Linux**: eliminar `~/.cache/legion` (o ejecutar `rm -rf ~/.cache/legion`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\legion`

3. Reinicie el escritorio Legion.

---

### Solucionar problemas de conexión del servidor

Legion Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `legion.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `LEGION_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `LEGION_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Legion requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Legion.

---

### Notificaciones que no se muestran

Legion Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Legion en la configuración de su sistema operativo, y
...
1. Salga del escritorio Legion.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Legion):

- `legion.settings.dat` (URL del servidor predeterminado de escritorio)
- `legion.global.dat` y `legion.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
2. **Únete a nuestro Discord**
   ...
   [**legion.dev/discord**](https://legion.dev/discord)
...
3. Asegúrese de tener la última versión con `legion upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `legion models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\legion`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. legion instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\legion`

2. Reinicie legion para reinstalar los paquetes del proveedor más recientes.

Esto obligará a legion a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
legion detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
