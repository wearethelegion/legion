---
title: Complementos
description: Escriba sus propios complementos para extender Legion.
---

Los complementos le permiten extender Legion al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Legion.
...
- `.legion/plugins/` - Complementos a nivel de proyecto
- `~/.config/legion/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/legion/node_modules/`.
...
1. Configuración global (`~/.config/legion/legion.json`)
2. Configuración del proyecto (`legion.json`)
3. Directorio global de complementos (`~/.config/legion/plugins/`)
4. Directorio de complementos del proyecto (`.legion/plugins/`)
...
Legion ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK legion para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar legion.
...
:::note
Si está utilizando la aplicación de escritorio Legion, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que legion lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a legion:
...
El ayudante `tool` crea una herramienta personalizada a la que legion puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para legion junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://legion.dev/docs/sdk) para obtener más detalles.
