...
description: Legion se integra con sus servidores LSP.
---

Legion se integra con su protocolo de servidor de idiomas (LSP) para ayudar a LLM a interactuar con su código base. Utiliza diagnósticos para proporcionar retroalimentación al LLM.

---

## Incorporados

Legion viene con varios servidores LSP integrados para idiomas populares:
...
:::note
Puede deshabilitar las descargas automáticas del servidor LSP configurando la variable de entorno `LEGION_DISABLE_LSP_DOWNLOAD` en `true`.
:::
...
Puede personalizar los servidores LSP a través de la sección `lsp` en su configuración legion.
