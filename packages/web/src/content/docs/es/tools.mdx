...
Las herramientas permiten que LLM realice acciones en su código base. Legion viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Legion.
...
:::note
Esta herramienta solo está disponible cuando `LEGION_EXPERIMENTAL_LSP_TOOL=true` (o `LEGION_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Legion o cuando la variable de entorno `LEGION_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Legion:

```bash
LEGION_ENABLE_EXA=1 legion
```
