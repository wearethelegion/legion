...
Puede agregar herramientas externas a Legion utilizando el _Model Context Protocol_ o MCP. Legion admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Legion Config](https://legion.dev/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/legion`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Legion maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Legion:
...
Si el servidor requiere autenticación, Legion le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `legion mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Legion almacenará los tokens de forma segura en `~/.local/share/legion/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Legion, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Legion como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Legion a su cuenta Sentry.
