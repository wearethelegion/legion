---
title: Configuration
description: Utilisation de la configuration JSON Legion.
---

Vous pouvez configurer Legion à l'aide d'un fichier de configuration JSON.

---

## Format

Legion prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
Par exemple, si votre configuration globale définit `theme: "legion"` et `autoupdate: true` et que la configuration de votre projet définit `model: "anthropic/claude-sonnet-4-5"`, la configuration finale inclura les trois paramètres.
...
1. **Configuration distante** (à partir de `.well-known/legion`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/legion/legion.json`) - préférences utilisateur
3. **Configuration personnalisée** (`LEGION_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`legion.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.legion`** - agents, commandes, plugins
6. **Configuration en ligne** (`LEGION_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.legion` et `~/.config/legion` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/legion`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/legion"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale Legion dans `~/.config/legion/legion.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les thèmes, les fournisseurs ou les raccourcis clavier.
...
Ajoutez `legion.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.

:::tip
Placez la configuration spécifique au projet à la racine de votre projet.
:::

Lorsque Legion démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `LEGION_CONFIG`.

```bash
export LEGION_CONFIG=/path/to/my/custom-config.json
legion run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `LEGION_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.legion`, et devrait suivre la même structure.

```bash
export LEGION_CONFIG_DIR=/path/to/my/config-directory
legion run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.legion`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`legion.dev/config.json`**](https://legion.dev/config.json).
...
Vous pouvez configurer les paramètres du serveur pour les commandes `legion serve` et `legion web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Legion.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `legion.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Legion via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Legion essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez configurer le thème que vous souhaitez utiliser dans votre configuration Legion via l'option `theme`.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/legion/agents/` ou `.legion/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Legion reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`legion run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/legion/commands/` ou `.legion/commands/`. [En savoir plus ici](/docs/commands).
...
Legion téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, legion **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Legion avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.legion/plugins/` ou `~/.config/legion/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="legion.json"
{
  "$schema": "https://legion.dev/config.json",
  "plugin": ["legion-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre Legion à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
