---
title: Serveurs LSP
description: Legion s'intègre à vos serveurs LSP.
---

Legion s'intègre via le protocole LSP (Language Server Protocol) pour aider le LLM à interagir avec votre base de code. Il utilise des diagnostics pour fournir des commentaires au LLM.

---

## Serveurs intégrés

Legion est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `LEGION_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque legion ouvre un fichier, il :
...
Vous pouvez personnaliser les serveurs LSP via la section `lsp` de votre configuration legion.
