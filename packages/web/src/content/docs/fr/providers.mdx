---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Legion.
---

import config from "../../../../config.mjs"
export const console = config.console

Legion utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Legion.
...
en `~/.local/share/legion/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Legion
configuration.
...
Legion Zen est une liste de modèles fournis par l'équipe Legion qui ont été
testé et vérifié pour fonctionner correctement avec Legion. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Legion Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez legion et dirigez-vous vers [legion.dev/auth](https://legion.dev/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Legion et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec Legion :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de legion :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY legion

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile legion

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX legion
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `legion.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Legion n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que legion fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX legion
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX legion
   ```
...
Vous pouvez également ajouter des modèles via votre configuration legion.
...
pour stocker le jeton dans le stockage d'authentification legion.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Legion utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Legion
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `legion.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### Configuration

Personnalisez via `legion.json` :
...
{
  "$schema": "https://legion.dev/config.json",
  "plugin": ["@gitlab/legion-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec legion :
...
Pour utiliser Google Vertex AI avec Legion :
...
Définissez-les lors de l'exécution de legion.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id legion
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via legion, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/legion/legion.jsonc"
{
  "$schema": "https://legion.dev/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) pour enregistrer automatiquement chaque conversation Legion en tant que session dans Helicone.

```bash
npm install -g legion-helicone-session
```
...
```json title="legion.json"
{
  "plugin": ["legion-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Legion répertoriée comme une session distincte.
...
Vous pouvez configurer legion pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="legion.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://legion.dev/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer legion pour utiliser des modèles locaux via LM Studio.

```json title="legion.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://legion.dev/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer legion pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Legion. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/legion) pour plus de détails.
:::

```json title="legion.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://legion.dev/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Legion :
...
3. Copiez la clé API à utiliser dans Legion.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Legion, vous devez extraire les informations du modèle localement :
...
Legion Zen est une liste de modèles testés et vérifiés fournie par l'équipe Legion. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Legion Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Legion Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Legion API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration legion.
...
5. Vous pouvez également les personnaliser via votre configuration legion. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' legion
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group legion
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Legion :
...
Vous pouvez également personnaliser les modèles via votre configuration legion. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec legion. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in legion.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `legion.json` dans le répertoire de votre projet :

   ```json title="legion.json" ""myprovider"" {5-15}
   {
     "$schema": "https://legion.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="legion.json" {9,11,17-20}
{
  "$schema": "https://legion.dev/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Legion de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `legion auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration legion et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration legion.
