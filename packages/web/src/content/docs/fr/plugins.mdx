---
title: Plugins
description: Écrivez vos propres plugins pour étendre Legion.
---

Les plugins vous permettent d'étendre Legion en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Legion.
...
- `.legion/plugins/` - Plugins au niveau du projet
- `~/.config/legion/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/legion/node_modules/`.
...
1. Configuration globale (`~/.config/legion/legion.json`)
2. Configuration du projet (`legion.json`)
3. Répertoire global des plugins (`~/.config/legion/plugins/`)
4. Répertoire des plugins du projet (`.legion/plugins/`)
...
Legion exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK legion pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre legion.
...
:::note
Si vous utilisez l'application de bureau Legion, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez legion de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à legion :
...
L'assistant `tool` crée un outil personnalisé que legion peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour legion aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://legion.dev/docs/sdk) pour plus de détails.
