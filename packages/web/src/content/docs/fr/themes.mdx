...
Avec Legion, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Legion utilise notre propre thème `legion`.
...
Legion est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Legion corresponde à l'apparence de leur terminal
...
Legion prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/legion/themes/*.json` ou `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.legion/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.legion/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/legion/themes
vim ~/.config/legion/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .legion/themes
vim .legion/themes/my-theme.json
```
...
{
  "$schema": "https://legion.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
