...
Il existe deux types d'agents dans Legion ; agents primaires et sous-agents.
...
Legion est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Legion est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Legion est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `legion.json` :
...
- Global : `~/.config/legion/agents/`
- Par projet : `.legion/agents/`

```markdown title="~/.config/legion/agents/review.md"
---
...
Si aucune température n'est spécifiée, Legion utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Legion et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Legion utilise le format `provider/model-id`. Par exemple, si vous utilisez [Legion Zen](/docs/zen), vous utiliserez `legion/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="legion.json" {3-6,9-12}
{
  "$schema": "https://legion.dev/config.json",
  "tools": {
    "write": true,
...
```json title="legion.json" {3-5,8-10}
{
  "$schema": "https://legion.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `legion models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/wearethelegion/legion).
:::
...
```markdown title="~/.config/legion/agents/docs-writer.md"
---
...
```markdown title="~/.config/legion/agents/security-auditor.md"
---
