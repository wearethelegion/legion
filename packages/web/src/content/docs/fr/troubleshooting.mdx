...
Pour déboguer les problèmes avec Legion, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\legion\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `legion --log-level DEBUG`.

---

## Stockage

legion stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/legion/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\legion`
...
Legion Desktop exécute un serveur Legion local (le side-car `legion-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Legion` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/legion/legion.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\legion\legion.jsonc`
...
Legion peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/legion/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\legion\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Legion puisse le reconstruire.

1. Quittez complètement Legion Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/legion`
- **Linux** : supprimez `~/.cache/legion` (ou exécutez `rm -rf ~/.cache/legion`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\legion`

3. Redémarrez le bureau Legion.

---

### Résoudre les problèmes de connexion au serveur

Legion Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `legion.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `LEGION_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `LEGION_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Legion Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Legion.

---

### Les notifications ne s'affichent pas

Legion Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Legion dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Legion.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Legion Desktop) :

- `legion.settings.dat` (serveur de bureau par défaut URL)
- `legion.global.dat` et `legion.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Legion :
...
3. Assurez-vous d'avoir la dernière version avec `legion upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `legion models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\legion`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. legion installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\legion`

2. Redémarrez legion pour réinstaller les derniers packages du fournisseur

Cela forcera legion à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
legion détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
