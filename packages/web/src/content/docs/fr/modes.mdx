...
:::caution
Les modes sont désormais configurés via l'option `agent` dans la configuration legion.
L’option `mode` est désormais obsolète. [En savoir plus](/docs/agents).
:::

Les modes dans legion vous permettent de personnaliser le comportement, les outils et les prompts pour différents cas d'utilisation.

Il est livré avec deux modes intégrés : **build** et **plan**. Vous pouvez personnaliser ceux-ci ou configurez les vôtres via la configuration legion.
...
legion est livré avec deux modes intégrés.
...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.legion/plans/*.md` pour détailler le plan lui-même
...
Configurez les modes dans votre fichier de configuration `legion.json` :
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Donc ça marche pour à la fois la configuration globale legion et la configuration spécifique au projet.
...
Créez des fichiers de mode dans `.legion/modes/` pour les modes spécifiques au projet ou `~/.config/legion/modes/` pour les modes globaux :
