---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć legion.
---

Wtyczki umożliwiające wyłączenie legion poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania legion.
...
- `.legion/plugins/` - Wtyczki na poziomie projektu
- `~/.config/legion/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/legion/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/legion/legion.json`)
2. Konfiguracja projektu (`legion.json`)
3. Globalny katalog wtyczek (`~/.config/legion/plugins/`)
4. Katalog wtyczek projektu (`.legion/plugins/`)
...
legion uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z legion do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do legion.
...
:::note
Jeśli korzystasz z aplikacji komputerowej legion, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj legion czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do legion:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod legion. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://legion.dev/docs/sdk).
