---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w legion.
---

import config from "../../../../config.mjs"
export const console = config.console

legion używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji legion.
...
w `~/.local/share/legion/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim legion
config.
...
Legion Zen to lista modeli dostarczonych przez zespół legion, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z legion. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Legion Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz legion i przejdź do [legion.dev/auth](https://legion.dev/auth).
...
Działa jak każdy inny dostawca w legion i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z legion:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania legion:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `legion.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w legion nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod legion działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji legion.
...
legion używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować legion
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`legion.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### Konfiguracja

Customize through `legion.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem legion:
...
Aby używać Google Vertex AI z legion:
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą legion, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session), aby automatycznie rejestrować każdą konwersację legion jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja legion będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować legion tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować legion tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować legion tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem legion. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/legion).
:::
...
Aby korzystać z Ollama Cloud z legion:
...
3. Skopiuj klucz API do użycia w legion.
...
6. **Ważne**: Przed użyciem modeli chmurowych w legion musisz pobrać informacje o modelu lokalnie:
...
Legion Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół legion. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Legion Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Legion Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API legion.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji legion.
...
5. Możesz je także dostosować za pomocą konfiguracji legion. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z legion:
...
Możesz także dostosować modele za pomocą konfiguracji legion. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z legion. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `legion.json` w katalogu projektu:
...
Pola `limit` pozwalają legion zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `legion auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację legion i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji legion.
