...
Dzięki legion możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie legion używa naszego własnego motywu `legion`.
...
legion ma kilka wbudowanych motywów.
...
- Want legion to match their terminal's appearance
...
legion obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/legion/themes/*.json` lub `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.legion/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.legion/themes/*.json`
