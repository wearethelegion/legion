...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji legion.
...
te lub skonfiguruj własne za pomocą konfiguracji legion.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.legion/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `legion.json`:
...
Jeśli nie ma wpływu na temperaturę, legion stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna legion, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.legion/modes/` dla trybów zapisanych dla projektu lub `~/.config/legion/modes/` dla trybów globalnych:
