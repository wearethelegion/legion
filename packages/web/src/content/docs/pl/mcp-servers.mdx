...
Możesz dodać dodatkowe informacje do legion za pomocą _Model Context Protocol_, w skrócie MCP. legion obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [legion Config](https://legion.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/legion`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
legion automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, legion:
...
Jeśli serwer wymaga uwierzytelnienia, legion poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `legion mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji legion bezpieczne przechowa tokeny w `~/.local/share/legion/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w legion, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie legion, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć legion z kontem Sentry.
