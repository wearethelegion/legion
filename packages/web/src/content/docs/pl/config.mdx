---
title: Konfiguracja
description: Ustawienia Legion w pliku JSON.
---

Możesz dostosować Legion za pomocą pliku konfiguracyjnego JSON.

---

## Format

Legion obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "legion"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/legion`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/legion/legion.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `LEGION_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`legion.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.legion`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `LEGION_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.legion` i `~/.config/legion` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/legion`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Legion w `~/.config/legion/legion.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak motywy, domyślny dostawca lub skróty klawiszowe.
...
Dodaj `legion.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.

:::tip
Dodaj ten plik do kontroli wersji, aby udostępniać konfigurację zespołowi.
:::

Kiedy Legion się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `LEGION_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `LEGION_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.legion` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.legion`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`legion.dev/config.json`**](https://legion.dev/config.json).
...
Możesz skonfigurować serwer dla `legion serve` i `legion web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Legion.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `legion.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Legion wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Skonfiguruj motyw interfejsu Legion za pomocą opcji `theme`.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/legion/agents/` lub `.legion/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Legion powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`legion run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/legion/commands/` lub `.legion/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Legion może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Legion nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Legion **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Legion o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.legion/plugins/` lub `~/.config/legion/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Legion tylko do określonych dostawców.
