...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Legion posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Legion.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `LEGION_EXPERIMENTAL_LSP_TOOL=true` (lub `LEGION_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Legion lub gdy zmienna środowiskowa `LEGION_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Legion:
