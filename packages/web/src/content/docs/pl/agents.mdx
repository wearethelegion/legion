...
W Legion obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Legion zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Legion ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Legion ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `legion.json`:
...
- Globalnie: `~/.config/legion/agents/`
- W projekcie: `.legion/agents/`
...
Jeśli nie zostanie skonfigurowana, Legion używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Legion, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Legion w `provider/model-id`. Na przykład, [Legion Zen](/docs/zen), użyj `legion/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `legion models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/wearethelegion/legion).
:::
