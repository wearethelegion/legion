...
legion ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak legion kendi `legion` temasını kullanır.
...
legion birden fazla yerleşik temayla gelir.
...
- legion'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
legion, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/legion/themes/*.json` veya `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.legion/themes/*.json`
4. **Geçerli çalışma dizini** - `./.legion/themes/*.json`
