...
legion ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\legion\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `legion --log-level DEBUG`.

---

## Depolama

legion, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/legion/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\legion` yapistirin
...
legion Desktop arka planda yerel bir legion sunucusu (`legion-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `legion` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/legion/legion.jsonc` (veya `~/.config/legion/legion.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/legion/legion.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\legion\legion.jsonc` yapistirin
...
legion diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/legion/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\legion\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), legion'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. legion Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/legion`
- **Linux**: `~/.cache/legion` dizinini silin (veya `rm -rf ~/.cache/legion` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\legion` yapistirin

3. legion Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

legion Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`legion.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `LEGION_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `LEGION_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta legion Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, legion ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

legion Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda legion icin bildirimler etkinse
...
1. legion Desktop'u kapatin
2. Su dosyalari bulun ve silin (legion Desktop uygulama veri dizininde yer alirlar):

- `legion.settings.dat` (desktop varsayilan sunucu URL'si)
- `legion.global.dat` ve `legion.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
legion ile ilgili bir sorun yasiyorsaniz:
...
3. `legion upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `legion models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\legion`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. legion, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\legion`

2. En guncel provider paketlerini yeniden kurmak icin legion'u yeniden baslatin

Bu, legion'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
legion, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
