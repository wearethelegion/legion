...
:::caution
Modlar artık legion yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

legion'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya legion yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
legion iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.legion/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`legion.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, legion modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global legion yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.legion/modes/` veya genel modlar için `~/.config/legion/modes/`'de mod dosyaları oluşturun:
