---
title: Yapılandırma
description: legion JSON yapılandırması kullanır.
---

legion'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

legion hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
Örneğin, genel ayarınız `theme: "legion"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
1. **Uzak yapılandırma** (`.well-known/legion`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/legion/legion.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`LEGION_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `legion.json`) - projeye özgü ayarlar
5. **`.legion` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`LEGION_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.legion` ve `~/.config/legion` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/legion` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global legion yapılandırmanızı `~/.config/legion/legion.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `legion.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

legion başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
`LEGION_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`LEGION_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.legion` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.legion` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`legion.dev/config.json`**](https://legion.dev/config.json)'da tanımlanan bir şemaya sahiptir.
...
`legion serve` ve `legion web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların legion sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `legion.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
legion yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak legion, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
legion yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/legion/agents/` veya `.legion/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, legion bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`legion run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/legion/commands/` veya `.legion/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
legion başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, legion açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) legion'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.legion/plugins/` veya `~/.config/legion/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, legion'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
