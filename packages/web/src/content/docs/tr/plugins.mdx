---
title: Eklentiler
description: legion'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek legion'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya legion'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.legion/plugins/` - Proje düzeyinde eklentiler
- `~/.config/legion/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/legion/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/legion/legion.json`)
2. Project config (`legion.json`)
3. Global eklenti dizini (`~/.config/legion/plugins/`)
4. Proje eklenti dizini (`.legion/plugins/`)
...
legion bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için legion'lu bir SDK istemcisi.
...
legion'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
legion masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
legion'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca legion'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, legion'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://legion.dev/docs/sdk)'e bakın.
