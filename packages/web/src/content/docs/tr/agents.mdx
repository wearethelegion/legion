...
legion'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
legion, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
legion, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

legion iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `legion.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse legion modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global legion yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
legion hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [Legion Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `legion/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `legion models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/wearethelegion/legion).
:::
