---
title: Sağlayıcılar
description: legion'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

legion, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı legion yapılandırmanızda yapılandırın.
...
`~/.local/share/legion/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları legion'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Legion Zen, legion ekibi tarafından sağlanan modellerin bir listesidir.
legion ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Legion Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, legion'u seçin ve [legion.dev/auth](https://legion.dev/auth)'ye gidin.
...
legion'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u legion ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   legion'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `legion.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi legion'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   legion'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Legion yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in legion auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
legion, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. legion'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`legion.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth) ana sayfasında.

##### Yapılandırma

`legion.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi legion'la kullanmak için:
...
Google Vertex AI'yi legion ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları legion çalıştırılırken ayarlayın.
...
Helicone'un legion aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her legion görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her legion görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
legion'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla legion'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla legion'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini legion için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/legion)'a bakın.
:::
...
Ollama Cloud'u legion ile kullanmak için:
...
3. legion'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: legion'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Legion Zen, legion ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Legion Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Legion Zen**'i arayın.

   ```txt
   /connect
   ```

3. legion API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   legion yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca legion yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Legion ile kullanmak için:
...
Modelleri legion yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı legion ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `legion.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları legion'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `legion auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için legion yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, legion yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
