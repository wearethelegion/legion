...
_Model Bağlam Protokolü_ veya MCP kullanarak legion'a harici araçlar ekleyebilirsiniz. legion hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [legion Config](https://legion.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/legion` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
legion, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde legion şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa legion, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `legion mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından legion, belirteçleri `~/.local/share/legion/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra legion'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi legion yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve legion'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
