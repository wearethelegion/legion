...
As ferramentas permitem que o LLM execute ações em sua base de código. O legion vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no legion.
...
:::note
Esta ferramenta está disponível apenas quando `LEGION_EXPERIMENTAL_LSP_TOOL=true` (ou `LEGION_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor legion ou quando a variável de ambiente `LEGION_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o legion:
