---
title: Configuração
description: Usando a configuração JSON do legion.
---

Você pode configurar o legion usando um arquivo de configuração JSON.

---

## Formato

O legion suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
Por exemplo, se sua configuração global define `theme: "legion"` e `autoupdate: true`, e sua configuração de projeto define `model: "anthropic/claude-sonnet-4-5"`, a configuração final incluirá as três configurações.
...
1. **Configuração remota** (de `.well-known/legion`) - padrões organizacionais
2. **Configuração global** (`~/.config/legion/legion.json`) - preferências do usuário
3. **Configuração personalizada** (`LEGION_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`legion.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.legion`** - agentes, comandos, plugins
6. **Configuração inline** (`LEGION_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.legion` e `~/.config/legion` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/legion`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do legion em `~/.config/legion/legion.json`. Use a configuração global para preferências de usuário, como temas, provedores ou atalhos de teclado.
...
Adicione `legion.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.

:::tip
Coloque a configuração específica do projeto na raiz do seu projeto.
:::

Quando o legion é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `LEGION_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `LEGION_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.legion`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.legion`, então ele **pode substituir** suas configurações.

---

## Esquema

O arquivo de configuração tem um esquema que está definido em [**`legion.dev/config.json`**](https://legion.dev/config.json).
...
Você pode configurar as configurações do servidor para os comandos `legion serve` e `legion web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor legion.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `legion.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do legion através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o legion tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você pode configurar o tema que deseja usar em sua configuração do legion através da opção `theme`.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/legion/agents/` ou `.legion/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o legion recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`legion run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/legion/commands/` ou `.legion/commands/`. [Saiba mais aqui](/docs/commands).
...
O legion fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o legion **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o legion com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.legion/plugins/` ou `~/.config/legion/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o legion para usar apenas provedores específicos, em vez de desabilitá-los um a um.
