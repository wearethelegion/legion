...
Com o legion, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o legion usa nosso próprio tema `legion`.
...
O legion vem com vários temas embutidos.
...
- Querem que o legion corresponda à aparência do seu terminal
...
O legion suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/legion/themes/*.json` ou `$XDG_CONFIG_HOME/legion/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.legion/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.legion/themes/*.json`
