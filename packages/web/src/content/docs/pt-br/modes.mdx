...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do legion. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no legion permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do legion.
...
O legion vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.legion/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `legion.json`:
...
- Global: `~/.config/legion/modes/`
- Projeto: `.legion/modes/`

```markdown title="~/.config/legion/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o legion usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do legion quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.legion/modes/` para modos específicos do projeto ou `~/.config/legion/modes/` para modos globais:

```markdown title=".legion/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/legion/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
