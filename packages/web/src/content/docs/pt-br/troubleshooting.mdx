...
Para depurar problemas com o legion, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\legion\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `legion --log-level DEBUG`.

---

## Armazenamento

O legion armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/legion/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\legion`
...
O legion Desktop executa um servidor legion local (o sidecar `legion-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Legion` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/legion/legion.jsonc` (ou `~/.config/legion/legion.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/legion/legion.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\legion\legion.jsonc`
...
O legion também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/legion/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\legion\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o legion possa reconstruí-lo.

1. Saia completamente do legion Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/legion`
- **Linux**: exclua `~/.cache/legion` (ou execute `rm -rf ~/.cache/legion`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\legion`

3. Reinicie o legion Desktop.

---

### Corrigindo problemas de conexão com o servidor

O legion Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `legion.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `LEGION_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `LEGION_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o legion Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do legion.

---

### Notificações não aparecendo

O legion Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o legion nas configurações do seu sistema operacional, e
...
1. Saia do legion Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo legion Desktop):

- `legion.settings.dat` (URL do servidor padrão do desktop)
- `legion.global.dat` e `legion.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o legion:
...
3. Certifique-se de que você tem a versão mais recente com `legion upgrade`
...
Para descobrir quais modelos você tem acesso, execute `legion models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\legion`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O legion instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\legion`

2. Reinicie o legion para reinstalar os pacotes de provedores mais recentes

Isso forçará o legion a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O legion detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
