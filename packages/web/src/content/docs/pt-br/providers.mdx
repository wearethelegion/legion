---
title: Provedores
description: Usando qualquer provedor LLM no legion.
---

import config from "../../../../config.mjs"
export const console = config.console

legion usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do legion.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/legion/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do legion.
...
Legion Zen é uma lista de modelos fornecidos pela equipe do legion que foram testados e verificados para funcionar bem com o legion. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Legion Zen.
:::

1. Execute o comando `/connect` no TUI, selecione legion e acesse [legion.dev/auth](https://legion.dev/auth).
...
Funciona como qualquer outro provedor no legion e é completamente opcional.
...
Para usar o Amazon Bedrock com o legion:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   Defina uma dessas variáveis de ambiente ao executar o legion:
   ...
   Para configuração específica do projeto ou persistente, use `legion.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no legion não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o legion funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do legion.
...
armazenar o token no armazenamento de autenticação do legion.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
legion usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o legion
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `legion.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### Configuração

Personalize através do `legion.json`:
...
Para usar sua assinatura do GitHub Copilot com o legion:
...
Para usar o Google Vertex AI com o legion:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o legion.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do legion, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session) para registrar automaticamente cada conversa do legion como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do legion listada como uma sessão separada.
...
Você pode configurar o legion para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o legion para usar modelos locais através do LM Studio.
...
Você pode configurar o legion para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o legion. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/legion) para detalhes.
:::
...
Para usar o Ollama Cloud com o legion:
...
3. Copie a chave da API para uso no legion.
...
6. **Importante**: Antes de usar modelos em nuvem no legion, você deve puxar as informações do modelo localmente:
...
Legion Zen é uma lista de modelos testados e verificados fornecidos pela equipe do legion. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Legion Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Legion Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do legion.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do legion.
...
5. Você também pode personalizá-los através da sua configuração do legion. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o legion:
...
Você também pode personalizar modelos através da sua configuração do legion. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o legion. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
1. Execute o comando `/connect` e role para baixo até **Outro**.
   ...
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no legion.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `legion.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o legion entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `legion auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do legion e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do legion.
