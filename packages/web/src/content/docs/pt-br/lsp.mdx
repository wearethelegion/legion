...
description: O legion integra-se aos seus servidores LSP.
---

O legion integra-se ao seu Protocolo de Servidor de Linguagem (LSP) para ajudar o LLM a interagir com sua base de código. Ele usa diagnósticos para fornecer feedback ao LLM.

---

## Integrados

O legion vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `LEGION_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o legion abre um arquivo, ele:
...
Você pode personalizar os servidores LSP através da seção `lsp` na sua configuração do legion.
