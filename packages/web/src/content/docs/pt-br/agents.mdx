...
Existem dois tipos de agentes no legion; agentes primários e subagentes.
...
legion vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
legion vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

legion vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `legion.json`:
...
Se nenhuma temperatura for especificada, o legion usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do legion quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do legion usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Legion Zen](/docs/zen), você usaria `legion/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `legion models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/wearethelegion/legion).
:::
