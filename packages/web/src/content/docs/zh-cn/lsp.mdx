---
title: LSP 服务器
description: Legion 与你的 LSP 服务器集成。
---

Legion 与你的语言服务器协议（LSP）集成，帮助 LLM 与你的代码库进行交互。它利用诊断信息向 LLM 提供反馈。

---

## 内置支持

Legion 内置了多种适用于主流语言的 LSP 服务器：
...
:::note
你可以将 `LEGION_DISABLE_LSP_DOWNLOAD` 环境变量设置为 `true` 来禁用 LSP 服务器的自动下载。
:::
...
你可以通过 legion 配置文件中的 `lsp` 部分来自定义 LSP 服务器。
