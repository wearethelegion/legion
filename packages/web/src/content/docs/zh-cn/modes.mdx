...
:::caution
模式现在通过 legion 配置中的 `agent` 选项进行配置。`mode` 选项已废弃。[了解更多](/docs/agents)。
:::

legion 中的模式允许你为不同的使用场景自定义行为、工具和提示词。

legion 自带两种内置模式：**build** 和 **plan**。你可以自定义这些模式，也可以通过 legion 配置创建自己的模式。
...
- `write` - 无法创建新文件
- `edit` - 无法修改现有文件，但位于 `.legion/plans/*.md` 的文件除外，用于详细说明计划本身
- `patch` - 无法应用补丁
- `bash` - 无法执行 shell 命令
...
此路径相对于配置文件所在位置。因此，全局 legion 配置和项目特定配置均可使用。
...
在 `.legion/modes/` 中创建项目特定的模式文件，或在 `~/.config/legion/modes/` 中创建全局模式文件：
