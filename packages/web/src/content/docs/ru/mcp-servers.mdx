...
Вы можете добавить внешние инструменты в legion, используя _Model Context Protocol_ или MCP. legion поддерживает как локальные, так и удаленные серверы.
...
Вы можете определить серверы MCP в своем [legion Config](https://legion.dev/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
...
Организации могут предоставлять серверы MCP по умолчанию через свою конечную точку `.well-known/legion`. Эти серверы могут быть отключены по умолчанию, что позволяет пользователям выбирать те, которые им нужны.
...
legion автоматически обрабатывает аутентификацию OAuth для удаленных серверов MCP. Когда серверу требуется аутентификация, legion:
...
Если сервер требует аутентификации, legion предложит вам пройти аутентификацию при первой попытке его использования. Если нет, вы можете [вручную запустить поток ](#authenticating) с помощью `legion mcp auth <server-name>`.
...
Команда `mcp auth` откроет ваш браузер для авторизации. После того как вы авторизуетесь, legion надежно сохранит токены в `~/.local/share/legion/mcp-auth.json`.
...
Ваши MCP доступны в виде инструментов legion наряду со встроенными инструментами. Таким образом, вы можете управлять ими через конфигурацию legion, как и любым другим инструментом.
...
Откроется окно браузера для завершения процесса OAuth и подключения legion к вашей учетной записи Sentry.
