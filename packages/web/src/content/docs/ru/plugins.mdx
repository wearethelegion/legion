---
title: Плагины
description: Напишите свои собственные плагины для расширения legion.
---

Плагины позволяют расширять legion, подключаясь к различным событиям и настраивая поведение. Вы можете создавать плагины для добавления новых функций, интеграции с внешними сервисами или изменения поведения legion по умолчанию.
...
- `.legion/plugins/` – плагины уровня проекта.
- `~/.config/legion/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/legion/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/legion/legion.json`)
2. Конфигурация проекта (`legion.json`)
3. Глобальный каталог плагинов (`~/.config/legion/plugins/`)
4. Каталог плагинов проекта (`.legion/plugins/`)
...
legion запускает `bun install` при запуске для их установки. Затем ваши плагины и инструменты смогут импортировать их.
...
Вот несколько примеров плагинов, которые вы можете использовать для расширения legion.
...
:::note
Если вы используете настольное приложение legion, оно может автоматически отправлять системные уведомления, когда ответ готов или когда возникает ошибка сеанса.
:::
...
Плагины также могут добавлять в legion собственные инструменты:
...
Помощник `tool` создает собственный инструмент, который может вызывать legion. Он принимает функцию схемы Zod и возвращает определение инструмента:
...
Уровни: `debug`, `info`, `warn`, `error`. Подробности см. в документации SDK](https://legion.dev/docs/sdk).
