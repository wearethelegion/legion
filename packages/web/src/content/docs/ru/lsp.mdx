---
title: LSP-серверы
description: legion интегрируется с вашими серверами LSP.
---

legion интегрируется с вашим протоколом языкового сервера (LSP), чтобы помочь LLM взаимодействовать с вашей кодовой базой. Он использует диагностику для предоставления обратной связи LLM.

---

## Встроенные

legion поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `LEGION_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда legion открывает файл, он:
...
Вы можете настроить серверы LSP через раздел `lsp` в конфигурации legion.
