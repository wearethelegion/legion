...
:::caution
Режимы теперь настраиваются с помощью опции `agent` в конфигурации legion.
Опция `mode` устарела. [Подробнее ](/docs/agents).
:::

Режимы в legion позволяют настраивать поведение, инструменты и подсказки для различных вариантов использования.

Он имеет два встроенных режима: **сборка** и **планирование**. Вы можете настроить
эти или настройте свои собственные через конфигурацию legion.
...
legion имеет два встроенных режима.
...
- `edit` – невозможно изменить существующие файлы, за исключением файлов, расположенных по адресу `.legion/plans/*.md`, для детализации самого плана.
...
Настройте режимы в файле конфигурации `legion.json`:
...
- Глобальный: `~/.config/legion/modes/`
- Проект: `.legion/modes/`
...
Если температура не указана, legion использует значения по умолчанию для конкретной модели (обычно 0 для большинства моделей, 0,55 для моделей Qwen).
...
как глобальная конфигурация legion, так и конфигурация конкретного проекта.
...
Создайте файлы режимов в `.legion/modes/` для режимов, специфичных для проекта, или в `~/.config/legion/modes/` для глобальных режимов:
