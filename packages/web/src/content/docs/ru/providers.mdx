---
title: Провайдеры
description: Использование любого провайдера LLM в legion.
---

import config from "../../../../config.mjs"
export const console = config.console

legion использует [AI SDK](https://ai-sdk.dev/) и [Models.dev](https://models.dev) для поддержки **более 75 поставщиков LLM** и поддерживает запуск локальных моделей.

Чтобы добавить провайдера, вам необходимо:

1. Добавьте ключи API для провайдера с помощью команды `/connect`.
2. Настройте провайдера в вашей конфигурации legion.
...
в `~/.local/share/legion/auth.json`.

---

### Настройка

Вы можете настроить поставщиков через раздел `provider` в вашем legion.
конфиг.
...
Legion Zen — это список моделей, предоставленный командой legion, которые были
протестировано и проверено на хорошую работу с legion. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с Legion Zen.
:::

1. Запустите команду `/connect` в TUI, выберите legion и перейдите по адресу [legion.dev/auth](https://legion.dev/auth).
...
Он работает как любой другой поставщик в legion и его использование совершенно необязательно.
...
Чтобы использовать Amazon Bedrock с legion:
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Установите одну из этих переменных среды при запуске legion:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `legion.json`:
...
:::info
Использование вашей подписки Claude Pro/Max в legion официально не поддерживается [Anthropic](https://anthropic.com).
:::
...
2. Перейдите в [Azure AI Foundry](https://ai.azure.com/) и разверните модель.
   ...
   :::примечание
   Для правильной работы legion имя развертывания должно совпадать с именем модели.
   :::
...
5. Запустите команду `/models`, чтобы выбрать модель.
   ...
   Вы также можете добавлять модели через конфигурацию legion.
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `Legion`, Области применения: `api`
...
для хранения токена в хранилище аутентификации legion.
:::

##### Самостоятельная GitLab

:::note[примечание о соответствии]
legion использует небольшую модель для некоторых задач ИИ, таких как создание заголовка сеанса.
По умолчанию он настроен на использование gpt-5-nano, размещенного на Zen. Чтобы заблокировать legion
чтобы использовать только свой собственный экземпляр, размещенный на GitLab, добавьте следующее в свой
`legion.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Дополнительная документация на домашней странице [legion-gitlab-auth](https://www.npmjs.com/package/@gitlab/legion-gitlab-auth).

##### Конфигурация

Настройте через `legion.json`:
...
Чтобы использовать Google Vertex AI с legion:
...
2. Установите необходимые переменные среды:
   ...
   Установите их во время запуска legion.
...
Если вы видите функцию или модель от Helicone, которая не настраивается автоматически через legion, вы всегда можете настроить ее самостоятельно.
...
Функция Helicone [Sessions](https://docs.helicone.ai/features/sessions) позволяет группировать связанные запросы LLM вместе. Используйте плагин [legion-helicone-session](https://github.com/H2Shami/legion-helicone-session), чтобы автоматически регистрировать каждый диалог legion как сеанс в Helicone.
...
Плагин вставляет в ваши запросы заголовки `Helicone-Session-Id` и `Helicone-Session-Name`. На странице «Сеансы» Helicone вы увидите каждый диалог legion, указанный как отдельный сеанс.
...
Вы можете настроить legion для использования локальных моделей с помощью [утилиты llama-server llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Вы можете настроить legion для использования локальных моделей через LM Studio.
...
Вы можете настроить legion для использования локальных моделей через Ollama.

:::tip
Ollama может автоматически настроиться для legion. Подробности см. в документации по интеграции Ollama](https://docs.ollama.com/integrations/legion).
:::
...
Чтобы использовать Ollama Cloud с legion:
...
3. Скопируйте ключ API для использования в legion.
...
6. **Важно**. Перед использованием облачных моделей в legion необходимо получить информацию о модели локально:
...
Legion Zen — это список протестированных и проверенных моделей, предоставленный командой legion. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>Legion Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **Legion Zen**.

   ```txt
   /connect
   ```

3. Введите свой ключ API legion.
...
4. Многие модели OpenRouter предварительно загружены по умолчанию. Запустите команду `/models`, чтобы выбрать нужную.
   ...
   Вы также можете добавить дополнительные модели через конфигурацию legion.
...
5. Вы также можете настроить их через конфигурацию legion. Вот пример указания провайдера
...
Чтобы использовать [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) с legion:
...
Вы также можете настраивать модели через конфигурацию legion. Ниже приведен пример указания порядка маршрутизации поставщика.
...
4. Создайте или обновите файл `legion.json` в каталоге вашего проекта:
...
Поля `limit` позволяют legion понять, сколько контекста у вас осталось. Стандартные поставщики автоматически извлекают их из models.dev.
...
1. **Проверьте настройку аутентификации**: запустите `legion auth list`, чтобы проверить, верны ли учетные данные.
...
2. Для пользовательских поставщиков проверьте конфигурацию legion и:
   - Убедитесь, что идентификатор провайдера, используемый в команде `/connect`, соответствует идентификатору в вашей конфигурации legion.
