...
Создайте `.legion/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию legion или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем legion [config](/docs/config):
...
- Глобальный: `~/.config/legion/commands/`
- Для каждого проекта: `.legion/commands/`
...
legion включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
