...
Legion has a list of keybinds that you can customize through the Legion config.
...
Legion uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Legion desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `legion.json`.
