...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("legion-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("legion_lib=debug,legion_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("legion_lib=info,legion_desktop=info,sidecar=info")
        }
    });
