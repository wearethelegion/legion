...
import {
  ...
  ServerConnection,
  useCommand,
} from "@wearethelegion/app"
import { Splash } from "@wearethelegion/ui/logo"
...
const deepLinkEvent = "legion:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__LEGION__ ??= {}
  const pending = window.__LEGION__.deepLinks ?? []
  window.__LEGION__.deepLinks = [...pending, ...urls]
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__LEGION__?.wsl) return undefined
    return commands.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__LEGION__?.wsl) return result
  ...
  return {
      ...
      await Promise.resolve()
        .then(() => {
          const notification = new Notification(title, {
            body: description ?? "",
            icon: "https://wearethelegion.com/favicon-96x96-v3.png",
          })
    ...
    getWslEnabled: async () => {
      ...
      return window.__LEGION__!.wsl ?? false
    },
