...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/wearethelegion/legion/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/wearethelegion/legion/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/legion packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/legion/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/legion/")) areas.add("core")
    ...
    .prompt(
      {
        ...
        model: { providerID: "legion", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], legion: Awaited<ReturnType<typeof createOpencode>>) {
  ...
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(batch.map((c) => summarizeCommit(legion, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/wearethelegion/legion/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  try {
    const lines = await generateChangelog(commits, legion)
    notes.push(...lines)
