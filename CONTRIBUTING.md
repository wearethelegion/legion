...
By default, `bun dev` runs Legion in the `packages/opencode` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Legion in the root of the opencode repo itself:
...
- Core pieces:
  - `packages/opencode`: Legion core business logic & server.
